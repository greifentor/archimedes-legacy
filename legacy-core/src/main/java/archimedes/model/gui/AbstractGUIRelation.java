...
import archimedes.gui.diagram.RectSegmentCalculator;
...
abstract public class AbstractGUIRelation implements GUIRelationModel {
	...
	public boolean collisionWithNeighbour(GUIViewModel view, Point p) {
		Rectangle r = new Rectangle(
				(int) p.getX() - HIT_TOLERANCE / 2,
				(int) p.getY() - HIT_TOLERANCE / 2,
				HIT_TOLERANCE,
				HIT_TOLERANCE);
	...
	public void computeOffset(GUIViewModel view, GUIObjectModel object, int x, int y) {
		...
		if ((mode == 0) || (mode == 1)) {
			RectSegmentCalculator rectSegmentCalculator = new RectSegmentCalculator();
			if (rectSegmentCalculator
					.getSegment(
							new Rectangle(
									(int) object.getX(view),
									(int) object.getY(view),
									(int) object.getWidth(),
	...
	public Point getPoint(GUIViewModel view, int x, int y) {
		...
		if (len > 0) {
			...
			for (int i = 1; i < len; i++) {
				...
				if (gp
						.intersects(
								(double) x - HIT_TOLERANCE / 2,
								(double) y - HIT_TOLERANCE / 2,
								HIT_TOLERANCE,
								HIT_TOLERANCE)) {
					p = new Point(x, y);
