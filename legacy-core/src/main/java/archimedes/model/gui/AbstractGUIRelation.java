...
abstract public class AbstractGUIRelation implements GUIRelationModel {
    ...
    private Hashtable<GUIViewModel, Direction[]> direction = new Hashtable<GUIViewModel,
            Direction[]>();
     ...
     */
    @Override public void addPoint(GUIViewModel view, Point p) {
     ...
     */
    @Override public void addPoint(GUIViewModel view, int x, int y) {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public boolean collisionWithNeighbour(GUIViewModel view, Point p) {
        Rectangle r = new Rectangle((int) p.getX() - HIT_TOLERANCE / 2, (int) p.getY() 
                - HIT_TOLERANCE  / 2, HIT_TOLERANCE, HIT_TOLERANCE);
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public void computeOffset(GUIViewModel view, GUIObjectModel object, int x, int y)
            {
        int mode = -1;
        if (this.getEndPoint(0) == this.getEndPoint(1)) {
            mode = this.lastMode;
            List<Point> lp = Arrays.asList(new Point[] {this.getPoint(view, 0),
                    this.getPoint(view, 1)});
            for (Point p : lp) {
                int xo = Math.max(p.x, x) - Math.min(p.x, x);
                int yo = Math.max(p.y, y) - Math.min(p.y, y);
                if ((xo <= GUIRelationModel.HIT_TOLERANCE)
                        && (yo <= GUIRelationModel.HIT_TOLERANCE)) {
                    mode = (p == lp.get(0) ? 0 : 1);
                    this.lastMode = mode;
                    break;
                }
        ...
        if ((mode == 0) || (mode == 1)) {
            ...
            Polygon p = new Polygon(new int[] {(int) object.getX(view)+1, (int) (object.getX(
                    view) + object.getWidth()-2), (int) (object.getX(view) + (object.getWidth()
                    / 2))}, new int[] {(int) object.getY(view)+1, (int) object.getY(view)+1,
                    (int) (object.getY(view) + (object.getHeight() / 2))}, 3);
            if (p.contains(x, y)) {
            ...
            p = new Polygon(new int[] {(int) object.getX(view)+1, (int) (object.getX(view)
                    + object.getWidth()-1), (int) (object.getX(view) + (object.getWidth() / 2))
            ...
            p = new Polygon(new int[] {(int) object.getX(view)+1, (int) object.getX(view)+1,
                    (int) (object.getX(view) + (object.getWidth() / 2))}, new int[] {(int)
                    object.getY(view)+1, (int) (object.getY(view) + object.getHeight()-2), (int)
                    (object.getY(view) + (object.getHeight() / 2))}, 3);
            if (p.contains(x, y)) {
            ...
            p = new Polygon(new int[] {h-1, h-1, h - (int) (object.getWidth() / 2)}, new int[] {
                    (int) object.getY(view)+1, (int) (object.getY(view) + object.getHeight()-2),
                    (int) (object.getY(view) + (object.getHeight() / 2))}, 3);
            if (p.contains(x, y)) {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public Point getPoint(GUIViewModel view, int x, int y) {
        Rectangle r = new Rectangle(x - HIT_TOLERANCE / 2, y - HIT_TOLERANCE  / 2, 
        ...
        if (len > 0) {
            ...
            for (int i = 1; i < len; i++) {
                ...
                if (gp.intersects((double) x - HIT_TOLERANCE / 2, (double) y - HIT_TOLERANCE 
                        / 2, HIT_TOLERANCE, HIT_TOLERANCE)) {
                    p = new Point(x, y);
     ...
     * @changed OLI 22.05.2013 - Added.
     */
    @Override public GUIRelationPoint[] getAllPointInformation() {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public Direction getDirection(GUIViewModel view, int i) {
     ...
     * @changed OLI 22.05.2013 - Added.
     */
    @Override public int getOffset(GUIViewModel view, int i) {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public List<Point> getPointsForView(GUIViewModel view) {
        List<Point> l = new Vector<Point>();
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public GUIObjectModel getReferencedObject() {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public GUIObjectModel getReferencerObject() {
     ...
     * @changed OLI 21.05.2013 - Added.
     */
    @Override public Point[] getShapePointsForView(GUIViewModel view) {
     ...
     * @changed OLI 21.05.2013 - Added.
     */
    @Override public List<GUIViewModel> getViews() {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public boolean isEndPoint(GUIViewModel view, Point p) {
        List<Point> ps = this.points.get(view);
        if (ps != null) {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public void removePoint(GUIViewModel view, Point p) {
        List<Point> ps = this.points.get(view);
        if (ps != null) {
            ps.remove(p);
        }
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public void setDirection(GUIViewModel view, int i, Direction d) {
        Direction[] da = this.direction.get(view);
        if (da == null) {
     ...
     * @changed OLI 17.05.2013 - Added.
     */
    @Override public void setOffset(GUIViewModel view, int i, int o) {
        int[] oa = this.offset.get(view);
        if (oa == null) {
            oa = new int[]{0, 0};
