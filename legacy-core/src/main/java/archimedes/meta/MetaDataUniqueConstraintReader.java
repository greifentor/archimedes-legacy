...
public class MetaDataUniqueConstraintReader {
     ...
     * @changed OLI 08.12.2015 - Added.
     */
    public void addUniqueConstraints(MetaDataModel model, JDBCDataSourceRecord connection,
    ...
    private void getUniqueMetaData(MetaDataModel model, Connection c) throws SQLException{
        ...
        for (int i = 0, leni = sv.size(); i < leni; i++) {
            ...
            rs = c.getMetaData().getIndexInfo(null, model.getSchemeName(), tableName, true,
                    false);
            while (rs.next()) {
                s = rs.getString("INDEX_NAME");
                if (pkNames.contains(s) || s.endsWith("_unique_index") || s.endsWith(
                        "_unique_index_nullable")) {
                    continue;
                }
