...
public class TablesTableModel extends AbstractTableModel {
    ...
    private archimedes.model.TableModel[] tables = null;
     ...
     * @changed OLI 23.10.2013 - Added.
     */
    public TablesTableModel(archimedes.model.TableModel[] tables,
            GUIBundle guiBundle) {
        super();
        this.tables = tables;
        this.guiBundle = guiBundle;
        for (archimedes.model.TableModel table : tables) {
    ...
    private boolean isTableUnselectedByStereoType(archimedes.model.TableModel table) {
        return false;
    }

    private boolean isCodeGenerationSuppressedByOption(archimedes.model.TableModel table) {
    ...
    @Override public void setValueAt(Object o, int row, int column) {
        if (column == 1) {
            ...
            this.tables[row].getDataModel().fireDataModelEvent(new TableChangedEvent(
                    this.tables[row]));
        }
    }
