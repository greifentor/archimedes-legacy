...
public class ModelCheckerDefaultValueForStringInvalid implements ModelChecker {
    ...
    public static final String RES_MODEL_CHECKER_INVALID_STRING_DEFAULT_LABEL
            = "ModelChecker.Messages.InvalidStringDefault.label";
                    ...
                    if (dv != null) {
                        if (!dv.startsWith("'") || !dv.endsWith("'")) {
                            l.add(new ModelCheckerMessage(ModelCheckerMessage.Level.ERROR,
                                    ...
                                    RES_MODEL_CHECKER_INVALID_STRING_DEFAULT_LABEL, c.getName(),
                                    t.getName(), dv), t));
                        }
                    }
                }
    ...
    boolean isStringType(DomainModel d) {
        int t = d.getDataType();
        if ((t == Types.CHAR) || (t == Types.LONGNVARCHAR) || (t == Types.LONGVARCHAR)
                || (t == Types.NCHAR) || (t == Types.NVARCHAR) || (t == Types.VARCHAR)) {
            return true;
        }
        return false;
    }
