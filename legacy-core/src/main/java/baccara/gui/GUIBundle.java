...
/**
 * A container class which creates a bundle of all objects which are required to create a GUI component.
 *
 * @author O.Lieshoff
 *
...
public class GUIBundle {
	 ...
	 *
	 * @param ini             The ini file which is used to store and read e. g. the components shape parameters.
	 * @param resourceManager The resource manager which provides the localized resources for the component.
	 ...
	 * @changed OLI 17.01.2013 - Added.
	 */
	public GUIBundle(Inifile ini, ResourceManager resourceManager, ImageProvider imageProvider, int hgap, int vgap)
	 ...
	 * @param actionListener An action listener which the button is to add to (or <CODE>null</CODE>, if the button is
	 *                       not to add to an action listener).
	 * @param panel          A panel which the button is to add to (or <CODE>null</CODE>, if the button should not be
	 *                       added to a panel).
	 * @return The new button.
	 * @throws IllegalArgumentException Passing a null pointer as resource id.
	 */
	public JButton createButton(String resourceId, String imageId, ActionListener actionListener, JPanel panel) {
		ensure(resourceId != null, "resource id cannot be null.");
		resourceId = this.getResourceIdentifier(resourceId + ".text", "General.button." + imageId + ".text")
				.replace(".text", "");
	 ...
	 * Creates a new button with the passed resource and image id.
	 *
	 * @param resourceId     A resource id for the button text (suffices for text, image and tooltip will be added
	 *                       automatically).
	 * @param actionListener An action listener which the button is to add to (or <CODE>null</CODE>, if the button is
	 *                       not to add to an action listener).
	 * @param panel          A panel which the button is to add to (or <CODE>null</CODE>, if the button should not be
	 ...
	 *
	 * @param id The id which assigns the text resource whose content should be shown in the label.
	 ...
	 * Returns a label with the content of the text resource assigned by the passed id.
	 *
	 * @param resourceId The id which assigns the text resource whose content should be shown in the label (only base
	 *                   id; ".text", ".image" and ".tooltip" will be added automatically.
	 * @return A <CODE>JLabel</CODE> with the text of the assigned text resource.
	 *
	 ...
	 * @param resourceId The id for the resource (base only; ".text", ".image" and ".mnemonic" will be added
	 *                   automatically.
	 ...
	 * @throws IllegalArgumentException Passing a null pointer as resource id for the menu item text.
	 *
	 * @changed OLI 20.05.2016 - Added.
	 */
	...
	/**
	 * Returns the passed resource id if there is a resource stored for or also no resource is found for the default id.
	 *
	 ...
	 * @return The passed resource id if there is a resource stored for or also no resource is found for the default id.
	 *
	...
	/**
	 * Returns the text of the resource with the passed id from the resource manager if there is a resource defined with
	 * the passed id (the id otherwise).
	 *
	 * @param id The id of the resource whose text is to return.
	 * @return text of the resource with the passed id from the resource manager if there is a resource defined with the
	 *         passed id (the id otherwise).
	 *
	 * @changed OLI 23.05.2013 - Added.
	 */
	 ...
	 *
	 * @param id       The id of the resource whose text is to return.
	 * @param replaces Some strings which will replace wild cards like "{0}" or "{1}" in the resource string.
	 * @return text of the resource with the passed id from the resource manager if there is a resource defined with the
	 *         passed id (the id otherwise).
	 *
	...
	public String getResourceText(String id, Object... replaces) {
		...
		if (replaces != null) {
			for (int i = 0; i < replaces.length; i++) {
				String r = (replaces[i] instanceof ResourceId ? ((ResourceId) replaces[i]).getResourceText(this)
						: String.valueOf(replaces[i]));
				s = s.replace("{" + i + "}", r);
			}
		}
		return s;
