...
import baccara.gui.ResourceId;
import corent.files.Inifile;
import corent.gui.FilenameSelector;
import corent.gui.FilenameSelectorComponentFactory;
import corent.gui.LineTextEditor;
import corent.gui.LineTextEditorComponentFactory;
import corent.gui.TimestampField;
import corent.gui.TimestampFieldComponentFactory;
...
public class EditorPanelCreator<T> {
	...
	public JPanel create(String resourcePrefix, Map<Component, T> componentsByComponent,
		...
		for (ComponentData<T> cd : components) {
			...
			} else if (cd.getType() == Type.FILE_NAME) {
				FilenameSelectorComponentFactory factory = new FilenameSelectorComponentFactory() {
					@Override
					public JButton createClearButton(FilenameSelector owner) {
						return guiBundle.createButton(crid + ".button.clear", "remove", null, null);
					}

					@Override
					public JButton createSelectButton(FilenameSelector owner) {
						return guiBundle.createButton(crid + ".button.select", "zoom", null, null);
					}

					@Override
					public JTextField createTextField(FilenameSelector owner) {
						return new JTextField(cd.getValue() != null
								? cd.getValue().toString()
								: "", 40);
					}
				};
				c = new FilenameSelector(cd.getValue() != null
						? cd.getValue().toString()
						: "", factory, guiBundle.getInifile(), resourcePrefix + ".selector");
