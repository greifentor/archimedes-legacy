 ...
 * <P>&Uuml;ber den Betriebsmodus (Objekte der Klasse InifileMode) k&ouml;nnen Objekte der 
 * Klasse veranla&szlig;t werden sich nach jeder &Auml;nderung oder nur bei Aufruf der Methode
 * <TT>save()</TT> in die angegebene Datei zu speichern.
 * <P><B>Anwendung:</B>
 * Eine Instanzierung der Klasse Inifile erzeugt zun&auml;chst nur das Inifile-Objekt und seine
 * bestimmt den Namen der mit ihm verbundenen Datei. Das eigentliche Einlesen der Datei wird 
 * &uuml;ber den Aufruf der Methode <TT>load()</TT> initiiert. Da die <TT>load()</TT>-Methode
 * die Exceptions der File-Operationen weiterreicht, mu&szlig; diese Aktion in einen 
 * entsprechenden try-catch-Block verpackt werden.
 * <PRE>
 ...
 * </PRE>
 *
 * <P>&Uuml;ber den Aufruf der Methode <TT>setInifileMode(InifileMode)</TT> kann der 
 * Betriebsmodus des Inifiles festgelegt werden. Die Defaulteinstellung hierzu ist 
 * <TT>InifileMode.SAVE_ON_DEMAND</TT>. Der Betriebsmodus kann auch nach dem Einlesen des 
 * Inifiles jederzeit ge&auml;ndert werden.
 *
 * <P>Zum Auslesen der Daten aus dem Inifile dienen die <TT>read...</TT>-Methoden. Den Methoden
 * wird immer ein Gruppenname, ein Attributname und ein Defaultwert &uuml;bergeben. Letzterer
 * wird zur&uuml;ckgeliefert, wenn zu der Gruppennamen-Attribut-Kombination kein Wert im
 * Inifile hinterlegt ist oder, wenn ein Typenconvertierungsproblem vorliegt.
 * <PRE>
 * <I>beispiel.ini</I>
 *
 * [Gruppe]
 ...
 * </PRE>
 *
 * <BR>F&uum;r die Beispieldatei bringen die folgenden exemplarischen read-Aufrufe die 
 * angegebenen Ergebnisse:
 *
 * <BR>readString("Gruppe", "Attribut", "Test") =&gt; "Inhalt".
 * <BR>readString("Gruppe", "AnderesAttribut", "Test") =&gt; "Test".
 * <BR>readLong("Gruppe", "Attribut", 4711) =&gt; 4711.
 *
 * <P>Mit Hilfe der <TT>write...</TT>-Methoden k&ouml;nnen Werte in das Inifile-Objekt 
 * &uuml;bernommen werden. Diese Methodenaufrufe m&uuml;ssen in try-catch-Bl&ouml;cke 
 * eingebettet werden. Dies ist durch die eventuell anfallende Speicherung bedingt, die im 
 * Betriebsmodus <TT>InifileMode.SAVE_ON_CHANGE</TT> durchgef&uuml;hrt wird.
 * <PRE>
 * try {
 ...
 * </PRE>
 *
 * <P>Um die Daten in die Datei zu &uuml;bernehmen, mu&szlig;, im Falle, da&szlig; das Inifile
 * nicht im Betriebsmodus <TT>InifileMode.SAVE_ON_CHANGE</TT> l&auml;uft, die Methode 
 * <TT>save()</TT> aufgerufen werden. Bei diesem Aufruf mu&szlig; die <TT>IOException</TT> 
 * abgefangen werden.
 * <PRE>
 * try {
 *     ini.save();
...
public class Inifile {
    ...
    /**
     * Generiert eine Inidatei und legt den Dateinamen anhand des &uuml;bergebenen Parameters 
     * fest. 
     *
    ...
    /**
     * Ermittelt den Wert eines Eintrages anhand des Schl&uuml;ssels aus Gruppe und 
     * Eintragsname. 
     *
     ...
     * @return Inhalt des benannten Eintrages bzw. <TT>null</TT>, wenn zu dem Schl&uuml;ssel 
    ...
    /**
     * Schreibt den &uuml;bergebenen Wert anhand des Schl&uuml;ssels aus Gruppe und Eintragsname
     * in die Inidatei. 
     *
     ...
     * @param value Der Wert, der unter dem angegebenen Schl&uuml;ssel in die Datei 
    ...
    /**
     * @return <TT>true</TT>, wenn der Inifilemode auf SAVE_ON_CHANGE eingestellt ist,
     *     <TT>false</TT> sonst.
     */
    ...
    /**
     * @return Eine TreeMap mit den zu der Inidatei geh&ouml;renden Gruppen, die nach ihrem 
     *     Namen geschl&uuml;sselt werden.
     */
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.lines.size(); i++) {
            ...
            if (o instanceof String) {
                s = (String) o;
                if (s.length() >= 3 && s.substring(0, 3).toUpperCase().equals("REM")) {
                    dump = dump.concat(s + Constants.LINEEND);
                } else if ((s.length() == 1) && (s.equals("\n"))) {
                    dump = dump.concat(Constants.LINEEND);
                } else {
                    if (sorted) {
                        s = (String) it.next();
                    }
                    dump = dump.concat(s + "=" + this.getEntry(group, s) + Constants.LINEEND);
                }
            } else if (o instanceof Group) {
                group = ((Group) o).getName();
                if (!group.equals(UNNAMEDGROUP)) {
                    dump = dump.concat("[" + group + "]" + Constants.LINEEND);
                }
                if (sorted) {
                    it = ((Group) o).getEntryIterator();
                }
     ...
     * @throws IOException wenn im WriteOnChangeMode ein Fehler beim Speichern der Datei 
     *     auftritt  
     */
    ...
    /**
     * Kopiert die Daten der &uuml;bergebenen Ini-Datei. Es wird lediglich die TreeMap mit den
     * Groups und deren Daten kopiert.
     *
