 ...
 * <P>
 * Das Verhalten der Methode GetObject(ResultSet, int, boolean) l&auml;&szlig;t sich durch folgende Properties
 * beeinflussen: <BR>
 * <TABLE BORDER=1>
 * <TR VALIGN=TOP>
 * <TH>Property</TH>
 ...
 * <TD>false</TD>
 * <TD>Boolean</TD>
 * <TD>Wird diese Property gesetzt, werden die Hochkommata in String (bei Nutzung der DBString-Methode) nicht in
 * datenbankspezifische Zeichen umgesetzt.</TD>
 * </TR>
 * <TR VALIGN=TOP>
 * <TD>corent.db.DBUtil.GetObject.force<BR>
 * .number.not.null</TD>
 * <TD>false</TD>
 * <TD>Boolean</TD>
 * <TD>Wird diese Property gesetzt, werden beim Lesen von Zahlenwerten &uuml;ber die Methode
 * <TT>GetObject(ResultSet, int, boolean)</TT> Null-Referenzen in echte, (runde) Nullwerte des entsprechenden Typs
 * umgesetzt.</TD>
 * </TR>
 * <TR VALIGN=TOP>
 * <TD>corent.db.DBUtil.GetObject.force<BR>
 * .numeric.to.double</TD>
 * <TD>false</TD>
 * <TD>Boolean</TD>
 * <TD>Wird diese Property gesetzt, so wird in der Methode <TT>GetObject(ResultSet, int,
 *             boolean)</TT> ein Datenfeld vom Typ <TT>numeric</TT> auch dann in ein Double-Objekt umgesetzt, wenn das
 * Datenfeld keine Nachkommastellen zul&auml;&szlig;t. Sonst werden nachkommenstellenlose Numerics in Long-Objekte
 * transformiert.
 * <P>
 * <I>Hinweis:</I> Bei HSQL-Datenbanken ist diese Property immer zu setzen. Hier ist ein Fehler im Datenbanktreiber
 * aufgefallen, der als Precision f&uuml;r Numerics immer 0 zur&uuml;ckliefert.</TD>
 * </TR>
 * <TR VALIGN=TOP>
 * <TD>corent.db.DBUtil.GetObject.treat<BR>
 * .longvarbinary.as.string</TD>
 * <TD>false</TD>
 * <TD>Boolean</TD>
 * <TD>Diese Property kann gesetzt werden, wenn der DB-Treiber LONGVARCHAR-Felder f&auml;lschlich als
 * LONGVARBINARY-Felder erkennt. Diese werden dann in einen String umgewandelt (Methode
 * <TT>GetObject(ResultSet, int, boolean)</TT>).</TD>
 * </TR>
 * <TR VALIGN=TOP>
 * <TD>corent.db.DBUtil.TypeByName</TD>
 * <TD>false</TD>
 * <TD>Boolean</TD>
 * <TD>Ein Setzen dieser Property schaltet die Typfindung auf den in den ResultSetMetaDaten angegebenen Namen des
 * Spaltendatentypen um. Sonst wird die numerische Typeninformation genutzt.</TD>
 * </TR>
 * </TABLE>
 * <HR>
 *
 ...
 * @changed OLI 15.04.2008 - Erweiterung der Methode <TT>GetObject(ResultSet, int, boolean)</TT> um die M&ouml;glichkeit
 *          LONGVARBINARY-Felder als String zu behandeln.
 * @changed OLI 29.09.2008 - Erweiterung um die Umsetzung von Hochkommata.
 * @changed OLI 19.02.2009 - Die Methode <TT>DBString(String)</TT> wandelt nun auch f&uuml;r PostgreSQL Backslashes um.
...
public class DBUtil {
	...
	/**
	 * Diese Methode liefert einen Datenbankstring (mit Hochkommata) zur&uuml;ck, falls es sich bei dem &uuml;bergebenen
	 * Java-String nicht um eine <TT>null</TT>-Referenz handelt. Sonst wird der String "NULL" zur&uuml;ckgegeben.
	 * <P>
	 * Beachten Sie, da&szlig; die Methode sich bez&uuml;glich der Hochkommata-Umsetzung an der DBMS-Konfiguration der
	 * statischen Instanz der DBExec-Klasse orientiert. <BR>
	 * Zudem ersetzt sie Backslashes in den Daten durch einen Doppelbackslash, sofern dies f&uuml;r das entsprechende
	 * DBMS notwendig ist. Dieser Austausch findet nur statt, wenn &uuml;ber die Property
	 * <TT>corent.db.DBUtil.DBString.suppress.quote.change</TT> ein Austausch der einfacher Hochkommata gegen
	 * Anf&uuml;hrungszeichen ablehnt wird.
	 *
	 * @param s Der umzuwandelnde Java-String.
	 * @return Die SQL-String-Entsprechung zum &uuml;bergebenen Java-String.
	 *
	 * @changed OLI 29.09.2008 - Erweiterung um die datenbankenspezifische Umsetzung von Hochkommata.
		...
		if (s != null) {
			if (!Boolean.getBoolean("corent.db.DBUtil.DBString.suppress.quote.change")) {
				if (DBExec.GetMode() == DBExecMode.HSQL) {
					s = s.replace("'", "\\apos");
				} else if ((DBExec.GetMode() == DBExecMode.MSSQL) || (DBExec.GetMode() == DBExecMode.MYSQL)) {
					s = s.replace("'", "\\'");
				} else if (DBExec.GetMode() == DBExecMode.POSTGRESQL) {
					s = s.replace("'", "''");
					s = s.replace("\\", "\\\\");
				} else {
					log.info("db mode " + DBExec.GetMode() + " does not support any changes for" + " quotes!");
				}
			} else {
				s = s.replace("'", "\"");
			}
	...
	/**
	 * Liefert den Types-Typen zum angegebene String bzw. eine IllegalArgumentException, wenn der String keinen
	 * g&uuml;tigen Typen enth&auml;lt.
	 *
	...
	public static int GetType(String s) {
		...
		if (s.equalsIgnoreCase("ARRAY")) {
			...
			return Types.CLOB;
			/*
			 * } else if (s.equalsIgnoreCase("DATALINK")) { return Types.DATALINK;
			 */
	 ...
	 * @param col          Die Spalte aus der das Objekt gelesen werden soll. (Nach ResultSet-Konvention beginnend mit
	 *                     Spalte 1 ...).
	 * @param nullsAllowed <TT>true</TT>, wenn als R&uuml;ckgabewert <TT>null</TT> zul&auml;ssig ist.
	 * @return Das Objekt.
	 * @throws SQLException Falls bei der Bearbeitung der Methode ein Fehler auftritt.
	 * 
	 * @changed OLI 27.12.2007 - Erweiterung um die Behandlung der Property
	 *          <I>corent.db.DBUtil.GetObject.force.numeric.to.double</I> (Boolean), mit deren Hilfe eine Umsetzung von
	 *          Numerics nach Double auch dann erzwungen wird, wenn sie keine Nachkommastellen haben.
	 *          <P>
	 *          OLI 15.04.2008 - Erweiterung um das wahlweise Lesen von LONGVARBINARY-Felder als LONGVARCHAR.
	 *          <P>
	 *          OLI 17.04.2008 - Erweiterung um die Property <I>corent.db.DBUtil.GetObject.force.number.not.null</I>.
	 *          Wird sie gesetzt, so werden Zahlenwerte beim Lesen einer Null-Referenz in echte (runde) Nullwerte
	 *          umgesetzt.
	 */
	public static Object GetObject(ResultSet rs, int col, boolean nullsAllowed) throws SQLException {
		...
		try {
			...
			case Types.DECIMAL:
				...
				if ((rs.getMetaData().getPrecision(col) == 0)
						&& !Boolean.getBoolean("corent.db.DBUtil.GetObject.force.numeric.to.double")) {
					obj = new Long(s);
				} else {
					obj = new Double(s);
				}
			...
			default:
				if ((columntype == Types.LONGVARBINARY)
						&& Boolean.getBoolean("corent.db.DBUtil.GetObject.treat.longvarbinary.as.string")) {
					obj = rs.getString(col);
				}
				break;
			}
