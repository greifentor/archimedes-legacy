...
import corent.base.Checker;
import corent.base.Constants;
import corent.base.StrUtil;
import corent.files.Inifile;
import corent.gui.DefaultAction;
import corent.gui.JDialogWithInifile;

/**
 * Diese Klasse bietet ein Fenster an, in dem ein Druckvorschau realisiert werden kann. <BR>
 * <HR>
 *
 * @author O.Lieshoff
 *
...
public class PrintPreview extends JDialogWithInifile {
	...
	/**
	 * Referenz auf den abgebildeten PrintJob (so es einen gibt, sonst null) <I>(Default null) </I>.
	 */
	protected CorePrintJob cpj = null;
	/**
	 * Flagge, die gesetzt wird, wenn der Dialog &uuml;ber den Abbruch-Button beendet wird <I>(Default false).
	 */
	...
	/**
	 * Generiert und &ouml;ffnet einen JDialog zur Anzeige des &uuml;bergebenen Objektes in einer Druckvorschau.
	 *
	 ...
	 * @param kontext Der Kontext f&uuml;r den Sicherheits- und Ressourcenmanager.
	 */
	public PrintPreview(Previewable pv, Inifile ini, Dimension dim, CorePrintJob cpj, String kontext) {
		super(ini);
	 ...
	 *
	 * @param dim Die Gr&ouml;&szlig;, falls es sich bei dem zweiten Parameter um eine PreviewComponent handelt.
	...
	private void construct(Dimension dim, Object c, CorePrintJob cpj, String kontext) {
		...
		if (c instanceof Previewable) {
			...
			panel
					.add(
							new JScrollPane(new JLabel(StrUtil.FromHTML("Anzeige wird nicht " + "unterst&uuml;tzt"))),
							BorderLayout.CENTER);
		}
		...
		if ((c instanceof Previewable) && (this.cpj != null) && (this.cpj.ja.getFromPage() < this.cpj.ja.getToPage())) {
	...
	/**
	 * @return Generiert einen Button, der w&auml;hrend der Dialogerstellung als Abbruch-Button eingef&uuml;gt wird.
	 */
	...
	/**
	 * @return Generiert einen Button, der w&auml;hrend der Dialogerstellung als Ok-Button eingef&uuml;gt wird.
	 */
	...
	/**
	 * @return Generiert einen Button, der w&auml;hrend der Dialogerstellung als 'Page-'-Button eingef&uuml;gt wird.
	 */
	...
	/**
	 * @return Generiert einen Button, der w&auml;hrend der Dialogerstellung als 'Zoom-'-Button eingef&uuml;gt wird.
	 */
	...
	/**
	 * @return <TT>true</TT>, wenn der Preview abgebrochen wurde,<BR>
	 *         <TT>false</TT> sonst.
	 */
	...
	private class ActionAbbruch extends DefaultAction {
	...
	private class ActionOk extends DefaultAction {
		protected ActionOk() {
			super("ActionOk");
		}
	...
	private class ActionPageMinus extends DefaultAction {
	...
	private class ActionPagePlus extends DefaultAction {
		private Previewable pv = null;
	...
	private class ActionZoomMinus extends DefaultAction {
	...
	private class ActionZoomPlus extends DefaultAction {
		private Zoomable z = null;
