...
/**
 * Dieser dient als Grundlage f&uuml;r die durch den GenGen erzeugten Generatoren. Er bietet
 * grundlegende Methoden, die durch die Ableitungen genutzt werden.
 * <P>Die eigentliche Implementierung des Interfaces <TT>CodeGenerator</TT> wird in den
 * abgeleiteten CodeGenerator-Klassen realisiert.
 *
 * <P><H3>Properties</H3>
 ...
 *         <TD>tmp/</TD>
 *         <TD>
 *             Der Pfad, auf dem die Code-Generierung basieren soll. Hier wird die
 *             Packagestruktur angelegt, unter der der CodeGenerator seine Klassen generiert.
 *         </TD>
 *     </TR>
 * </TABLE>
 *
 ...
 * @changed OLI 28.09.2009 - Anpassungen an den CLI-Betrieb. Einbau der property-gesteuerten
 *         Angabe des Basisverzeichnisses f&uuml;r die Codegenerierung.
 * @changed OLI 04.10.2009 - Aufhebung der Methode <TT>getSpaces(int)</TT>.
 * @changed OLI 07.10.2009 - Die Methode <TT>generate(MetaDataModell)</TT> schreibt keine leeren
 *         Dateien mehr. Stattdessen wird eine Warnung im Log erzeugt. Diese Eigenschaft kann
 *         dazu genutzt werden, um anhand der Klassenmetadaten zu entscheiden, ob Code f&uuml;r
 *         die Klasse generiert werden soll.
 * @changed OLI 07.10.2009 - Die Methode <TT>getDefaultValue(AttributeMetaData)</TT> liefert
 *         nun auch f&uuml;r elementare Boolean-Werte korrekte Werte, wenn die Defaultwerte
 *         in der Schreibweise nicht korrekt sind oder numerisch ausgedr&uuml;ckt werden.
 * @changed OLI 19.10.2010 - Erweiterung um die Methode <TT>isCodeClass(boolean)</TT> und deren
 *         Einbindung in die Pr&uuml;fung, ob Code f&uuml;r eine Klasse erzeugt werden soll.
 *         Einbindung des PTime-Typs.
 */
...
public abstract class AbstractCodeGenerator implements CodeGenerator {
     ...
     *
     * @param amd Die Attributmetadaten zu dem Attribute zu dem der Gettername generiert werden
     *         soll.
     * @return Der Gettername zum Attribut.
     * @throws NullPointerException Falls die Attributmetadaten als <TT>null</TT>-Referenz
     *         angegeben werden.
     *
     ...
     * @precondition cmd != <TT>null</TT>
     *
     * @deprecated OLI 30.09.2009 - Bitte die gleichnamige Methode aus der Utility-Klasse
     *         <TT>gengen.util.SQLUtil</TT> nutzen.
     */
     ...
     * @return Ein String mit dem Defaultwert zu den angegebenen Attributmetadaten zum
     *         Einf&uuml;gen in den generierten Code. F&uuml;r Attribute mit <TT>null</TT> als
     *         Defaultwert wird der String 'null' zur&uuml;ckgegeben. String werden mit
     *         Anf&uuml;hrungszeichen zur&uuml;ckgeliefert.
     *         <BR>F&uuml;r Attribute vom Typ <TT><B>boolen</B></TT> gilt: ein Defaultwert von
     *         "0" oder "false" (Gro&szlig;- und Kleinschreibung wird nicht beachtet) ergibt den
     *         String "false"; alles andere den String "true".
     *
    ...
    /**
     * Liefert ein Session-Tag f&uuml;r die angegebene Klasse zur Integration in eine
     + EJB-JAR-XML-Datei.
     *
     ...
     * @param indent Die Einr&uuml;ckung der &auml;&szlig;eren linken Linie des generierten
     *         codes.
     ...
     * @throws NullPointerException Falls die Klassenmetadaten oder der Basispackagename als
    ...
    public String getEJBJarSessionTag(ClassMetaData cmd, String bpn, int indent)
        ...
        String s = spc + "<session>\n"
            ...
            + spc + "    <remote>$PN$.scheme.ejb.interfaces.$PROJECTTOKEN$Master"
            + "DataInterface$CLASSNAME$Remote</remote>\n"
            + spc + "    <local-home>$PN$.scheme.ejb.interfaces.$PROJECTTOKEN$Master"
            + "DataInterface$CLASSNAME$LocalHome</local-home>\n"
            + spc + "    <local>$PN$.scheme.ejb.interfaces.$PROJECTTOKEN$MasterData"
            + "Interface$CLASSNAME$LocalRemote</local>\n"
            + spc + "    <ejb-class>$PN$.scheme.ejb.beans.$PROJECTTOKEN$MasterData"
            + "Interface$CLASSNAME$Bean</ejb-class>\n"
            + spc + "    <session-type>Stateless</session-type>\n"
            + spc + "    <transaction-type>Container</transaction-type>\n"
            + spc + "</session>\n";
        s = s.replace("$PN$", bpn);
    ...
    /**
     * Liefert ein Return-Statement f&uuml;r einen Vergleich der Attribute zweier Instanzen der
     * angegebenen Klasse.
     *
     * @param cmd Die Klassenmetadaten, anhand derer der Attributblock generiert werden soll.
     * @param vn Der Name der Variablen, auf die sich der Vergleich beziehen soll.
     * @return Eine Liste mit den durch Kommata abgesetzten Namen der Attribute der Klasse.
     * @throws NullPointerException Falls die Klassenmetadaten oder der angegebene Variablenname
     *         als <TT>null</TT>-Pointer &uuml;bergeben werden.
     *
     * @precondition cmd != <TT>null</TT> &amp;&amp; vn != <TT>null</TT>
     *
     * @changed OLI 21.09.2009 - Hinzugef&uuml;gt.
     */
    public String getEqualsReturnStatement(ClassMetaData cmd, String vn)
            throws NullPointerException {
        assert cmd != null : "class meta data can not be null for generating an equals return "
                + "statement.";
        ...
        for (i = 0, len = amds.size(); i < len; i++) {
            ...
            sb.append("Utl.equals(this.").append(an).append("(), ").append(vn).append("."
                    ).append(an).append("())");
            /*
            // OLI 07.10.2009 - Das war die alte, differenzierte Version.
            if (this.isElementaryType(amd.getJavaType())) {
                sb.append("(this.").append(an).append("() == ").append(vn).append(".").append(an
                        ).append("())");
            } else {
                sb.append("this.").append(an).append("().equals(").append(vn).append("."
                        ).append(an).append("())");
            }
            */
        }
     ...
     * @param id Die Id der Spalte des ResultSet, aus dem die Daten f&uuml;r das Attribut
     *         gelesen werden sollen.
     * @return Ein Codefragment zum Einlesen des Attributs aus einem REsultSet (rs).
     * @throws NullPointerException Falls die Attributmetadaten als <TT>null</TT>-Pointer
     *         &uuml;bergeben werden.
     *
     * @precondition amd != <TT>null</TT>
     */
    public String getReadStatement(AttributeMetaData amd, int indent, int id) {
        assert amd != null : "attribute meta data can not be null for generating a read "
                + "statement";
        String defaultLineClose = id + "));\n";
        StringBuffer sb = new StringBuffer(this.getSpaces(indent)).append("o.set").append(
                amd.getName()).append("(");
    ...
    /**
     * Generiert ein Java-Codefragment, mit dem f&uuml;r das angegbene Attribut von der
     * angegebenen Position aus einem ResultSet gelesem werden kann.
     * <BR>Die erzeugten Codefragmente sind auf die Nutzung in Kombination mit dem
     * AbstractCodeGenerator und einer AbstractDBDataSource-Implementierung abgestimmt.
     *
     * <P><I><B>Beispiel:</B> <TT>rs.getString(10)</TT> oder <TT>this.getNumber(rs, 42, 1L)</I>
     *
     ...
     *         hier eine <TT>null</TT>-Referenz &uuml;bergeben, so wird der Name "rs" f&uuml;r
     *         das ResultSet angenommen und in den Code generiert.
    ...
    public String getRSGet(AttributeMetaData amd, int i, String vn)
        ...
        sb = new StringBuffer();
        if (amd.getJavaType().equals("boolean")) {
            return sb.append("(").append(vn).append(".getInt(").append(i).append(") != 0)"
                    ).toString();
        } else if (amd.getJavaType().equals("String")) {
            return sb.append(vn).append(".getString(").append(i).append(")").toString();
        } else if (amd.getJavaType().endsWith("LongPTimestamp")) {
            return sb.append("DBDataSourceUtil.getLongPTimestamp(").append(vn).append(
                    ", ").append(i).append(")").toString();
        } else if (amd.getJavaType().endsWith("PDate")) {
            return sb.append("DBDataSourceUtil.getPDate(").append(vn).append(", "
                    ).append(i).append(")").toString();
        } else if (amd.getJavaType().endsWith("PTime")) {
            return sb.append("DBDataSourceUtil.getPTime(").append(vn).append(", "
                    ).append(i).append(")").toString();
        } else if (amd.getJavaType().endsWith("PTimestamp")) {
            return sb.append("DBDataSourceUtil.getPTimestamp(").append(vn).append(", "
                    ).append(i).append(")").toString();
        } else if (gengen.util.SQLUtil.isElementaryNumberType(amd.getJavaType())) {
            return sb.append("DBDataSourceUtil.getNumber(").append(vn).append(", ").append(i
                    ).append(", ").append((amd.getDefaultValue() != null ? amd.getDefaultValue(
                    ).toString() + gengen.util.SQLUtil.getTypeSuffix(amd.getJavaType()) : "0L")
                    ).append(")").toString();
        }
        log.info("type '" + amd.getJavaType() + "' (" + amd.getName() + ") can not be processed"
                + " by AbstractCodeGenerator (method getRSGet(AttributeMetaData amd, int i, "
                + "String vn))");
        return "<<<<< MANUAL CODING REQUIERED (" + amd.getJavaType() + ") >>>>>";
    }

    /**
     * Liefert ein Codefragment mit einem Setter, der das angegebene Attribut aus einer
     * Stringvariablen bel&auml;dt.
     *
     * @param amd Das Attribut, zu dem der Setter gebildet werden soll.
     * @param vn Der Name der Stringvariablen, aus der der Setter den Wert in dem Codefragment
     *         lesen soll.
     * @param tsAsLong Wird diese Flagge gesetzt, so werden Zeitstempelsetter mit umgewandelten
     *         Longwerten (in entsprechenden Konstruktoren) aufgerufen.
     * @return Ein Codefragment mit einem Setter zum Attribut, f&uuml;r den der
     *         &Uum;bergabeparameter aus einem String konvertiert wird.
     * @throws IllegaleArgumentException Falls der Name der Stringvariablen leer &uuml;bergeben
     *         wird.
     * @throws NullPointerException Falls die Attributmetadaten oder der Name der
     *         Stringvariablen als <TT>null</TT>-Referenz &uuml;bergeben wird.
     * @precondition amd != <TT>null</TT>.
    ...
    public static String getSetterFromString(AttributeMetaData amd, String vn, boolean tsAsLong)
        ...
        if (tsAsLong && (amd.getJavaType().endsWith("LongPTimestamp") || amd.getJavaType(
                ).endsWith("PDate") || amd.getJavaType().endsWith("PTimestamp"))) {
            sb.append("new ").append(amd.getJavaType()).append("(Long.valueOf(").append(vn
                    ).append(")));");
        } else {
            sb.append(AbstractCodeGenerator.getWrapperType(amd.getJavaType())).append(
                    ".valueOf(").append(vn).append("));");
        }
        return sb.toString();
    }
     ...
     * @return Ein Wrappertyp, falls es sich bei dem Javatyp um einen elementaren Typen handelt,
     *         den angegebenen Typen sonst.
     ...
     * @changed OLI 04.10.2009 - Hinzugef&uuml;gt.
     */
    public static String getWrapperType(String tn) throws IllegalArgumentException,
    ...
    /**
     * Pr&uuml;ft, ob Code f&uuml;r die Klasse erzeugt werden soll. Die Methode sollte
     * &uuml;berschrieben, falls eine Einschr&auml;nkung bei der Erzeugung des Codes
     * erw&uuml;nscht ist. Standardm&auml;&szlig;ig wird der Wert <TT>true</TT>
     * zur&uuml;ckgeliefert.
     *
     * @param cmd Die Klassenmetadaten, zu denen die Pr&uuml;fung stattfinden soll.
     * @return <TT>true</TT>, falls der Code f&uuml;r die Klasse erzeugt werden soll,
     *         <TT>false</TT> sonst.
     * @throws NullPointerException Falls die Klassenmetadaten als <TT>null</TT>-Pointer
     *         &uuml;bergeben werden.
     * @precondition cmd != <TT>null</TT>
     ...
     *
     * @todo * - Es ist zu &uuml;berlegen, ob die Methode hier nicht vollkommen herausfliegt,
     *         verstatict wird oder einfach so belassen wird (trotz Redundanz mit der Methode
     *         in der Klasse <TT>gengen.util.SQLUtil</TT> (OLI, 30.09.2009).
     */
    ...
    /**
     * Tauscht ein paar Standardbezeichner in Bezug auf eine Klasse aus. Die g&uuml;tigen
     * Bezeichner sind:<PRE>
     * $AUTHOR$            - Der Autor der Klasse.
     * $CLASSNAME$         - Der Klassenname.
     * $COMPLETECLASSNAME$ - Der vollst&auml;ndige Klassenname.
     * $DATE$              - Das aktuelle Tagesdatum.
     *
     ...
     * @throws NullPointerException Wenn der &uuml;bergebene Source-Code oder die
     *         Klassenmetadaten als Null-Referenz &uuml;bergeben werden.
     *
    ...
    public StringBuffer replaceClassWildCards(StringBuffer source, ClassMetaData cmd) {
        ...
        s = s.replace("$CLASSNAME$", cmd.getName());
        s = s.replace("$COMPLETECLASSNAME$", (this.getCompleteClassName(cmd) == null ? "<null>"
                : this.getCompleteClassName(cmd)));
        s = s.replace("$DATE$", new PDate().toString());
        return new StringBuffer(s);
    }

    /**
     * Tauscht ein paar Standardbezeichner in Bezug auf das Modell aus. Die g&uuml;tigen
     * Bezeichner sind:<PRE>
     * $BASEPACKAGE$   - Der Name des Basispackages der Anwendung.
     * $VENDOR$        - Der Herstellername (z. B. f&uuml;r Copyrightvermerke).
     *
     * @param source Der bisher erzeugte Source-Code zur Klasse.
    ...
    /**
     * Setzt den Basispfad, unter dem die generierten Dateien gespeichert werden sollen auf den
     * angegebenen Wert.
     *
    ...
    /**
     * @changed OLI 07.10.2009 - Leere Codedateien werden nicht mehr geschrieben. Stattdessen
     *         gibt's eine Meldung im Log.
     * @changed OLI 19.10.2010 - M&ouml;glichkeit der Einschr&auml;nkung der Erzeugung des
     *         Codes.
     */
    public void generate(ModelMetaData mmd) throws Exception {
        ...
        String baseCodePath = FileUtil.completePath(Utl.getProperty(
                "gengen.generator.base.path", "tmp/")).replace("~", System.getProperty(
                "user.home"));
        String code = null;
        String fn = null;
        ...
        for (i = 0, len = cmds.size(); i < len; i++) {
            ...
            if (this.isCodeClass(cmd)) {
                ...
                if (this.getPackageName() != null) {
                    packagePath = packagePath.concat(FileUtil.completePath(this.getPackageName(
                            ).replace(".", "/")));
                }
                new File(this.getBasePath() + packagePath).mkdirs();
                fn = this.getBasePath() + packagePath + this.getCompleteClassName(cmd)
                        + ".java";
