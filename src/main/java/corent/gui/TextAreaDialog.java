 ...
 * <P>&Uuml;ber die Property <I>corent.gui.TextAreaDialog.ctrl</I> lassen sich zwei 
 * unterschiedliche Komfortmodi f&uuml;r das Eingabefeld einstellen. Wird der Wert "HTML" 
 * gesetzt so kann &uuml;ber die Tastenkombinationen <TT>CTRL-B</TT>, <TT>CTRL-I</TT> und 
 * <TT>CTRL-U</TT> ein entsprechendes HTML-Tag an der Cursorposition eingef&uuml;gt werden. Wird
 * hingegen der Wert "$HTML" gesetzt, so werden &uuml;ber die gleichen Tastenkombinationen 
...
public class TextAreaDialog extends JDialogWithInifile {
     ...
     *     TextArea-Dialoges herstellt.
     * @param text Der Text, der in der TextArea editiert werden soll.
     * @param enabled Diese Flagge ist zu setzen, wenn das Texteingabefeld nicht abgeblendet 
     *     werden soll.
     */
     ...
     * @param enabled Diese Flagge ist zu setzen, wenn das Texteingabefeld nicht abgeblendet 
     *     werden soll.
     * @param ini Die Inidatei, aus der der TextAreaDialog seine Gestalt rekonstruieren soll.
     */
    public TextAreaDialog(TextAreaDialogComponentFactory tadcf, String text, boolean enabled,
            Inifile ini) {
     ...
     */
    public TextAreaDialog(TextAreaDialogComponentFactory tadcf, String text, boolean enabled,
            Inifile ini, KeyListener keyListener) {
        ...
        this.textArea.addKeyListener(new KeyAdapter() {
            @Override public void keyPressed(KeyEvent e) {
                if (e.isControlDown() && (System.getProperty("corent.gui.TextAreaDialog.ctrl") 
                        != null)) {
                    if (e.getKeyCode() == KeyEvent.VK_B) {
                        int pos = textArea.getCaretPosition();
                        if (System.getProperty("corent.gui.TextAreaDialog.ctrl"
                                ).equalsIgnoreCase("HTML")) {
                            textArea.setText(StrUtil.InsertStr(textArea.getText(), "<B></B>", 
                                    pos));
                            textArea.setCaretPosition(pos+3);
                        } else if (System.getProperty("corent.gui.TextAreaDialog.ctrl"
                                ).equalsIgnoreCase("$HTML")) {
                            textArea.setText(StrUtil.InsertStr(textArea.getText(), 
                                    "$<B$>$</B$>", pos));
                        ...
                        if (System.getProperty("corent.gui.TextAreaDialog.ctrl"
                            ...
                            textArea.setText(StrUtil.InsertStr(textArea.getText(), "<I></I>", 
                                    pos));
                            textArea.setCaretPosition(pos+3);
                        } else if (System.getProperty("corent.gui.TextAreaDialog.ctrl"
                                ).equalsIgnoreCase("$HTML")) {
                        ...
                        if (System.getProperty("corent.gui.TextAreaDialog.ctrl"
                            ...
                            textArea.setText(StrUtil.InsertStr(textArea.getText(), "<U></U>", 
                                    pos));
                            textArea.setCaretPosition(pos+3);
                        } else if (System.getProperty("corent.gui.TextAreaDialog.ctrl"
                                ).equalsIgnoreCase("$HTML")) {
        ...
        if (System.getProperty("corent.resource.type", "none").equalsIgnoreCase("properties")) {
            ...
            if ((t != null) && (t.length() > 0)) { 
                System.out.println("title for -> " + cn);
                this.setTitle(t);
            }
        }
    ...
    /** Diese Methode wird aufgerufen, wenn der Benutzer den Verwerfen-Button dr&uuml;ckt. */
