...
/**
 * Mit Hilfe dieser Klasse werden die Komponenten der einzelnen Tabs des
 * EditorDjinns verwaltet.
 *
 * @author O.Lieshoff
 *         <P>
 *
 * @changed OLI 01.10.2008 - Erweiterung um Debugausgaben (zuschaltbar &uuml;ber
 *          "corent.djinn.debug").
 *          <P>
 */
...
public class ComponentTable {
	...
	/**
	 * F&uuml;gt die &uuml;bergebene DefaultEditorDjinnCell in die ComponentTable
	 * ein, wenn sie f&uuml;r das angegeben Tab noch nicht enthalten ist.
	 *
	 * @param tab  Das Tab, auf dem die Komponente eingef&uuml;gt werden soll.
	 * @param dedc Die DefaultEditorDjinnCell, aus der die Daten zur Produktion der
	 *             Komponente und deren Umfeld gewonnen werden soll.
	 *
	 * @changed OLI 01.10.2008 - Erweiterung um Debugausgaben (zuschaltbar &uuml;ber
	 *          "corent.djinn.debug").
	 *          <P>
	...
	public void addCell(int tab, DefaultEditorDjinnCell dedc) {
		...
		log.debug("ComponentTable: add for tab " + tab);
		if (v != null) {
			if (!v.contains(dedc)) {
				v.addElement(dedc);
			}
			log.debug("ComponentTable:     adding new tab");
		} else {
			v = new Vector();
			v.addElement(dedc);
			this.components.put(it, v);
			log.debug("ComponentTable:     appending to existing tab");
		}
	}
	 ...
	 * @throws IllegalArgumentException Falls zu der angegebenen (tab,
	 *                                  n)-Kombination keine DefaultEditorDjinnCell
	...
	public DefaultEditorDjinnCell getCell(int tab, int n) throws IllegalArgumentException {
		...
		throw new IllegalArgumentException("(tab, n) combination (" + tab + ", " + n + ") is not valid");
	}

	/** @return Anzahl der Tabs. */
	public int getTabCount() {
		log.debug("ComponentTable: returning tab count: " + this.components.size());
		return this.components.size();
	}

	/**
	 * @param tab Das Tab zu dem die Anzahl der gespeicherten
	 *            DefaultEditorDjinnCells ermittelt werden soll.
	 * @return Die Anzahl der zu dem Tab gespeicherten DefaultEditorDjinnCells, bzw.
	 *         0, wenn die Liste entweder leer ist oder keine Cells enth&auml;lt.
	 */
	 ...
	 * @param index Der Index, zu dem die Zelle aus der Liste gel&ouml;scht werden
	 *              soll.
	 */
