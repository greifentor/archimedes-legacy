...
/**
 * Mit Hilfe dieser Klasse k&ouml;nnen Druckauftr&auml;ge erzeugt, konfiguriert und 
 * ausgef&uuml;hrt werden. Sie bietet zudem die M&ouml;glichkeit Druckformulare aus 
 * beschreibenden Dateien zu &uuml;bernehmen und mit &uuml;bergebenen Daten zu f&uuml;llen.<BR>
 * <HR>
 *
 * @author O.Lieshoff
 *
...
public class CorePrintJob implements Previewable {
    ...
    /** 
     * Die Abst&auml;nde zu den Seitenr&auml;ndern in dots <I>(Default double[] {0.0d, 0.0d, 
     * 0.0d, 0.0d})</I>. 
     */
    ...
    /** 
     * Die PreviewComponent, in der der Prinjob gegebenenfalls dargstellt werden soll <I>(
     * Default) </I>.
     */
    protected PreviewComponent pc = null;
    /** 
     * Referenz auf den systemeigenen PrintJob, &uuml;ber den das Objekt seine Ausgabe tut <I>
     * (Default null)</I>.
     */
    ...
    /** 
     * Ist diese Flagge gesetzt werden die Zeichenanweisungen tats&auml;chlich ausgef&uuml;hrt,
     * andernfalls werden lediglich die dazu notwendigen Aktionen simuliert <I>(Default 
     * true)</I>.
     */
     ...
     * @throws IOException falls ein Fehler bei der Arbeit mit dem Dateisystem auftritt. 
     */
    public CorePrintJob(JobAttributes ja, PageAttributes pa, String fn, Hashtable params) 
            throws IOException { 
        this(ja, pa);
        this.params = params;
        this.configure(this.getScript(fn));
    ...
    public int dots(double cm) {
        return (int) ((double) this.dpcm() * cm);
    }
    
    /** @return Die dots per cm. */
    private int dpcm() {
        return (int) ((double) dpi / 2.54);
    }
    ...
    protected int margin(int d) {
        if ((d >= BOTTOM) && (d <= TOP)) {
            return (int) (this.margins[d] * (double) this.dpcm());
        }
        return 0;
    }
    
    /**
     * @return <TT>true</TT>, wenn die Koordinaten auf den Mittelpunkt der zu zeichnenden Figur
    ...
    public void drawImage(Graphics g, ImageIcon image, double x, double y) {
        int ix = (int) ((double) this.dpcm() * x); 
        int iy = (int) ((double) this.dpcm() * y);
        int ih = (int) ((double) image.getImage().getHeight(image.getImageObserver()) 
                * ((double) this.dpi) / (double) this.getPrinterResolution(1));
        int iw = (int) ((double) image.getImage().getWidth(image.getImageObserver()) 
                * ((double) this.dpi) / (double) this.getPrinterResolution());
    ...
    public void drawLine(Graphics g, double x, double y, double x0, double y0) {
        int ix = (int) ((double) this.dpcm() * x); 
        int iy = (int) ((double) this.dpcm() * y);
        int ix0 = (int) ((double) this.dpcm() * x0); 
        int iy0 = (int) ((double) this.dpcm() * y0);
        if (this.isDrawing()) {
    ...
    public void drawRect(Graphics g, double x, double y, double w, double h) {
        int ix = (int) ((double) this.dpcm() * x); 
        int iy = (int) ((double) this.dpcm() * y);
        int iw = (int) ((double) this.dpcm() * w); 
        int ih = (int) ((double) this.dpcm() * h);
        if (this.getAlign() == CENTER) {
            ix = ix - (int) (w / 2); 
        } else if (this.getAlign() == RIGHT) {
            ix = ix - (int) (w); 
    ...
    /** 
     * Schreibt den angegebenen String an die &uuml;bergebenen Koordinaten. Ist der String 
     * l&auml;nger als die angegebene Maximall&auml;nge, so werden die &uuml;berz&auml;hligen
     * Zeichen abgetrennt und drei Punkte zur Kennzeichnung angebracht.
     *
    ...
    /** 
     * Schreibt den angegebenen String an die &uuml;bergebenen Koordinaten. Ist der String 
     * l&auml;nger als die angegebene Maximall&auml;nge, so wird er umgebrochen und in der 
     * n&auml;chsten Zeile weitergef&uuml;hrt. Reicht die dazu angegebene Maximalh&ouml;he nicht
     * aus, so wird der String entsprechend gek&uml;rzt und um drei Punkte zur Kennzeichnung 
     * erweitert.
     *
     * @param g Der Graphics-KOntext, auf den geschrieben werden soll.
     * @param s Der zu schreibende String.
    ...
    public void drawString(Graphics g, String[] s, double x, double y, double w, double h, 
        ...
        for (int si = 0; si < s.length; si++) { 
            ...
            if ((s[si].length() == 0)) { 
                y0 = y;
                y = y + (((double) g.getFontMetrics().getHeight()) * za / ((double) 
                        this.dpcm()));
    ...
    /**
     * Zeichnet ein gef&uuml;lltes Rechteck mit abgerundeten Ecken an die angegebenen 
     * Koordinaten.
     *
     * @param g Der Grafikkontext, auf den gemalt werden soll.
     * @param x Die X-Koordinate der rechten oberen Ecke des Rechtecks.
     ...
     */
    public void fillRoundRect(Graphics g, double x, double y, double w, double h, double rw, 
            double rh) {
        int ix = (int) ((double) this.dpcm() * x); 
        int iy = (int) ((double) this.dpcm() * y);
        int iw = (int) ((double) this.dpcm() * w); 
        int ih = (int) ((double) this.dpcm() * h);
        int irw = (int) ((double) this.dpcm() * rw); 
        int irh = (int) ((double) this.dpcm() * rh);
        if (this.getAlign() == CENTER) {
            ix = ix - (int) (w / 2); 
        } else if (this.getAlign() == RIGHT) {
            ix = ix - (int) (w); 
     ...
     *
     * @param g Der Grafik-Kontext, f&uuml;r den die Einstellung vorgenommen werden soll.
     * @param fontname Der Name des Fonts.
     * @param style Der Style, in dem der Font angezeigt werden soll (PLAIN, ITALIC etc.).
    ...
    /**
     * Liest und entfernt die Konfigurationsanweisungen aus dem Script und arbeitet den Rest 
     * f&uuml;r die paint-Routine auf.
     *
     * @param script0 Das noch unbearbeitete Script.
     * @return <TT>true</TT>, wenn der Konfigurationsteil des Scriptes fehlerfrei abgearbeitet
     *     werden konnte,<BR><TT>false</TT> sonst.
     */
    protected boolean configure(Vector script0) {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (recording) {
                ...
                try {
                    if (s.equals("BOTTOM")) {
                        stack.push(new Double((double) BOTTOM));
                    } else if (s.equals("CENTER")) {
                        stack.push(new Double((double) CENTER));
                    } else if (s.equals("dpi")) {
                        this.changeDPI(((Double) stack.pop()).intValue());
                    } else if (s.equals("IMAGED")) {
                        stack.push(new Double((double) IMAGED));
                    } else if (s.equals("LEFT")) {
                        stack.push(new Double((double) LEFT));
                        ...
                        stack.push(new Double((double) NATIVE));
                    } else if (s.equals("RIGHT")) {
                        stack.push(new Double((double) RIGHT));
                    } else if (s.equals("start")) {
                        break;
                    } else if (s.equals("TOP")) {
                        stack.push(new Double((double) TOP));
        ...
        System.out.println("script cleaned");
        return code;
    }
     ...
     * &uuml;berschrieben werden. Auf Fehler bei der abarbeitung des Scriptes mu&szlig; mit 
     * einer Exception reagiert werden.
     * 
     * @param cmd Das Kommando.
     * @param stack Der Stack, auf dem das Script arbeitet.
     * @param i Die Position des Kommandos im Script.
     * @return <TT>true</TT>, wenn das Kommando erkannt worden ist,<BR><TT>false</TT> sonst.
     */
     ...
     *
     * @return <TT>true</TT>, falls der Auftrag ordnungsgem&auml;&szlig; ausgef&uuml;hrt werden
     *     konntem,<BR><TT>false</TT>, sonst.
     */
    ...
    public int countPages() {
        return this.makePages(new BufferedImage(this.dimPage.width, this.dimPage.height, 
                        BufferedImage.TYPE_INT_RGB).createGraphics(), -1);
    }
    ...
    public int makePages(Graphics g, int page) {
        if (this.script != null) {
            ...
            this.lasty = 0.0;
            if (this.mode == IMAGED) {
                img = new BufferedImage(this.dimPage.width, this.dimPage.height, 
            ...
            len = this.script.size();
            System.out.println("processing printscript ... (" + len + " params)");
            ...
            for (int i = startline; i < len; i++) {
                ...
                try {
                    if (s.equals("do")) {
                        ...
                        if ((int) x == 0) {
                            int count = 1;
                            while ((i+1 < len) && !s.equals("else") && !s.equals("endif") 
                                    && (count > 0)) {
                                i++;
                                s = (String) this.script.elementAt(i);
                                if (s.equals("if")) {
...
System.out.println("\n\nhpage" + this.hpage);
    ...
    protected void execute(String s, Graphics g, Stack stack, int line, int page) {
        ...
        if (s.equals("align")) {
            ...
            d = ((Double) stack.pop()).doubleValue() * ((double) this.dpi) / 72.0; 
            int size = (int) d;
            g.setFont(new Font(name, style, size));                        
        } else if (s.equals("height")) {
            Dimension dim = this.dimPage;
            stack.push(new Double(dim.getHeight() - this.margins[TOP] 
                    - this.margins[BOTTOM])); 
            ...
            Dimension dim = this.dimPage;
            stack.push(new Double(dim.getHeight() - this.margins[LEFT] 
                    - this.margins[RIGHT]));
        } else if (s.equals("?")) {
                ...
                for (int j = 0; j < lenj; j++) {
                    ...
                    if (o != null) {
                        ...
                        if (s0.equals("do")) {
                            ...
                            if ((int) x == 0) {
                                int count = 1;
                                while ((j+1 < lenj) && !s0.equals("else") && !s0.equals("endif") 
                                        && (count > 0)) {
                                    j++;
                                    s0 = (String) m0.getAnweisungen().elementAt(j);
                                    if (s0.equals("if")) {
     ...
     * gegebenenfalls auf den graphischen Kontext. Als Seiteneffekt wird der &uuml;bergebene
     * Stack manipuliert. Auf Fehler mu&szlig; mit einer Exception reagiert werden.
     *
     * @param cmd Das Kommando.
     * @param g Das Graphics-Objekt, auf dem die Ausgabe stattfinden soll.
     * @param stack Der Stack, von dem das Kommando seine Parameter beziehen kann.
    ...
    protected void printError(String message, String s, int param) {
        System.out.println("    " + message + (((s == null) && (param < 0)) ? "" : " -> " + s 
                + " (param #" + param + ")"));
    }

    /**
     * L&ouml;&szlig;st den &uuml;bergebenen Vector aus Anweisungszeilen in einzelne Strings 
     * auf.
     *
