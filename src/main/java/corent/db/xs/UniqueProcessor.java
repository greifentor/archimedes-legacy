...
/**
 * Diese Klasse bietet eine Funktion, die aus einem Einzigartigkeits-Ausdruck
 * aus einem PersistenceDescriptor eine SQL-Where-Klausel macht. Der
 * Einzigartigkeitsausdruck mu&szlig; nach folgender EBNF gebildet sein:
 * 
 * <PRE>
...
public class UniqueProcessor {
	...
	private static final Logger log = Logger.getLogger(UniqueProcessor.class);
	...
	private String spaltenname(Stack<Character> st) {
		...
		while (!st.empty() && (((st.peek() >= '0') && (st.peek() <= '9')) || ((st.peek() >= 'a') && (st.peek() <= 'z'))
				|| ((st.peek() >= 'A') && (st.peek() <= 'Z')) || (st.peek() == '_'))) {
	...
	private String andausdruck(Stack<Character> st, Hashtable<String, Object> attributes) {
		...
		if (value != null) {
			...
			log.warn("value in UniqueProcessor.andausdruck(Stack, Hashtable) is null!");
	...
	private String ausdruck(Stack<Character> st, Hashtable<String, Object> attributes) {
		...
		if (st.peek() == '(') {
			...
			if (value != null) {
				...
				log.warn("value in UniqueProcessor.ausdruck(Stack, Hashtable) is null!");
				erg = erg.concat(" is null");
			}
			if (this.nextOperator(st, '&')) {
				st.pop();
				erg = "(".concat(erg).concat(" and ").concat(this.andausdruck(st, attributes)).concat(")");
			}
			if (this.nextOperator(st, '|')) {
				st.pop();
	...
	/**
	 * Diese Methode wertet die dem Konstruktor &uuml;bergebene Formel aus und
	 * erstellt eine auf das &uuml;bergebene Persistent-Object zugeschittene
	 * Unique-Where-Klausel.
	 *
				...
				if (o != null) {
					if ((o instanceof String) && cr.isKodiert()) {
						if (DBFactoryUtil.CODER == null) {
							log.warn("Kodierung kann nicht durchgefuehrt werden wenn DBFactoryUtil.CODER == null in "
