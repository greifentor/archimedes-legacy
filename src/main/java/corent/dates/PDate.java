...
/**
 * Diese Klasse ist aus der urspr&uuml;nglich im Rahmen diverser privater Projekte entwickelten 
 * Klasse PDate hervorgegangen, die letztendlich im corelib-Package gelandet ist. Diese Klasse 
 * weist jedoch einige Probleme in bezug auf Sicherheit und Zuverl&auml;ssigkeit auf, 
 * soda&szlig; an dieser Stelle neu implementiert worden ist. Die Urspr&uuml;nge der Klasse 
 * liegen in einem Modula-2-Modul namens PackedDate, das im Rahmen Zahlreicher Mickey-D's 
 * Programme seinen Dienst tat und tut.
 * <P>Die Klasse repr&auml;sentiert ein Datum im Integer-Format (JJJJMMTT). Dies ist zwar keine 
 * unbedingt nat&uuml;rliche Datumsdarstellung, hat jedoch einen Wust an Vorteilen, der diese 
 * Art der Repr&auml;sentation rechtfertigt.<BR>
 * <P>Die Methoden der Klasse sind stellenweise auf hohe Fehlertoleranz angelegt. So errechnet 
 * das Verschieben von Daten um ganze Monate immer richtige Datum-Werte, die aber eventuell 
 * nicht den Erwartungen entsprechen k&ouml;nnen. Beispiel:<PRE>
 *  
 * monthBefore(1) vom 31.03.2003 -> 28.02.2003.
 * </PRE>
 * 
 * <P>&Uuml;ber die Property <TT>corent.dates.debug</TT> l&auml;&szlig;t sich an einigen Stellen
 * eine zus&auml;tzliche Debug-Ausgabe auf die Konsole einschalten.
 * <P>Mit Hilfe der Properties <TT>corent.dates.PDate.undefined.string.regular</TT> und 
 * <TT>corent.dates.PDate.undefined.string.short</TT> k&ouml;nnen ein alternative String zur 
 * Ausgabe eines undefinierten PDates in der regul&auml;ren und der kurzen Schreibweise 
 * angegeben werden. Die Defaultwerte sind "XX.XX.XXXX" und "XX.XX.XX".
 *
 * <P>Mit Hilfe der Properties <I>corent.dates.debug</I> und <I>corent.debug</I> k&ouml;nnen
 * zus&auml;tzliche Debuginformationen auf der Konsole ausgegeben werden.
 *
 * @author 
 *     O.Lieshoff
 *     <P>
 *
 * @changed
 *     OLI 14.11.2008 - Korrektur des Konstruktors <TT>PDate(java.util.Date)</TT>. Erweiterung
 *             um zuschaltbare Debugausgaben in diesem Konstruktor.
 *     <P>OLI 16.02.2009 - Erste Schritte zur Umstellung auf ein englischsprachiges zur Klasse.
 *     <P>OLI 14.04.2009 - Debugging an der Methode <TT>GetMontagVonWoche(int, int)</TT>.
 *     <P>
 *
 */
...
public class PDate implements Serializable {
    ...
    public PDate() {
        ...
        d =(dt.get(Calendar.YEAR) * 10000) + ((dt.get(Calendar.MONTH)+1) * 100) + dt.get(
                Calendar.DAY_OF_MONTH);
    }

    /**
     * Erzeugt ein PDate aus dem &uuml;bergebenen Integer-Wert, falls dieser ein g&uuml;ltiges 
     * PDate enth&auml;t.
     *
     * @param pd Ein int-Wert im PDate-Format.
     * @throws DateFormatException wenn der int-Wert kein g&uuml;ltiges Datum enth&auml;lt.
     */
    public PDate(int pd) throws DateFormatException {
        this(pd % 100, (pd / 100) % 100, pd / 10000);
    }
    /*
    public PDate(Integer ihd) throws DateFormatException {
        this(ihd.intValue() % 100, (ihd.intValue() / 100) % 100, ihd.intValue() / 10000);
    }
    */
     ...
     * @throws DateFormatException wenn der Inhalt des &uuml;bergebenen PDates fehlerhaft ist.
     */
     ...
     * @throws DateFormatException wenn der Inhalt des &uuml;bergebenen Dates fehlerhaft ist.
     *
     * @changed
     *     OLI 14.11.2008 - Fehler Beseitigt (Umwandlung wurde aufgrund einer Fehlern Klammerung
     *             der Monatsberechnung nicht korrekt durchgef&uuml;hrt). Au&szlig;erdem 
     *             Erweiterung um Debugausgaben.
     *     <P>
     *
     */
    public PDate(Date d) {
        ...
        Calendar dt = Calendar.getInstance();
        dt.setTime(d);
        if (CheckDate(dt.get(Calendar.DAY_OF_MONTH), dt.get(Calendar.MONTH)+1, dt.get(
                Calendar.YEAR))) {
            this.d = dt.get(Calendar.YEAR) * 10000 + (dt.get(Calendar.MONTH)+1) * 100 + dt.get(
                    Calendar.DAY_OF_MONTH);
            if (Boolean.getBoolean("corent.dates.debug") || Boolean.getBoolean("corent.debug"))
                    {
                System.out.println("\n\n PDate > " + this.d);
                System.out.println(" year  > " + dt.get(Calendar.YEAR) * 10000);
                System.out.println(" month > " + (dt.get(Calendar.MONTH)+1) * 100);
                System.out.println(" day   > " + dt.get(Calendar.DAY_OF_MONTH));
            }
        } else {
     ...
     * @throws DateFormatException wenn der Inhalt des &uuml;bergebenen String fehlerhaft ist.
     */
    ...
    /** 
     * Wandelt ein PDate in einen PTimestamp um. Die Uhrzeit des PTimestamps wird auf 0:00 Uhr
     * gesetzt.
     *
     * @return Ein PTimestamp mit dem Datum des PDates und der Uhrzeit 0:00 Uhr.
     * @throws java.text.ParseException Falls der Inhalt des PDates zu einem fehlerhaften 
     *         PTimestamp f&uuml;hren w&uuml;rde. Eigentlich d&uuml;rfte diese Exception an 
     *         dieser Stelle niemals geworfen werden.
     *
     * @changed
    ...
    public PTimestamp toPTimestamp() throws java.text.ParseException {
        ...
        return new PTimestamp(((long) this.toInt()) * 1000000l);
    }
    ...
    public String toString() {
        String s = "";
        if (this.toInt() == -1) {
            s = s + Utl.GetProperty("corent.dates.PDate.undefined.string.regular", 
                    "XX.XX.XXXX");
        } else {
            String h = "";
    ...
    private PDate shiftJahr(int n) {
        ...
        if (this.isSchaltjahr() && (!d.isSchaltjahr()) && (this.getMonat() == 2) 
                && (this.getTag() == 29)) {
            d = d.setTag(28);
        }
        return d;
    ...
    /** 
     * Ermittelt den Abstand zwischen dem vorliegenden und dem angegebenen Datum in Jahren.
     * 
    ...
    /**
     * Ermittelt den Abstand zwischen dem vorliegenden und dem angegebenen Datum in Tagen.
     * 
    ...
    public int getWoche() {
        boolean debug = Boolean.getBoolean("corent.dates.debug") || Boolean.getBoolean(
                "corent.debug");
        if (debug) {
        ...
        if (debug) {
            ...
            System.out.println("    dt.get(Calendar.WEEK_OF_YEAR)=" 
                    + dt.get(Calendar.WEEK_OF_YEAR));
        }
        return dt.get(Calendar.WEEK_OF_YEAR);
    }

    /**
     * Ermittelt, ob sich das vorliegende Datum zwischen den beiden angegebenen Daten befindet.
     * Liegt das Datum d0 hinter dem Datum d1, so werden die Daten vertauscht.
     *
     * @param d0 Beginn des Zeitraums.
     * @param d1 Ende des Zeitraumes.
     * @return <TT>true</TT>, wenn das vorliegende Datum in dem angegebenen Zeitraum liegt.
     */
     ...
     *
     * @param woche Die Nummer der Woche, zu der der Montag ermittelt werden soll (sollte 
     *         zwischen 1 und 53 liegen).
     * @param jahr Die Jahreszahl, um die Woche eindeutig zu beschreiben.
     * @return Das Datum des Montags der Woche als PDate.
     * @throws IllegalArgumentException falls die Woche au&szlig;erhalb der zul&auml;ssigen 
     *         Parameter liegt.
     *
     * @changed
     *     OLI 14.04.2009 - Hinzugef&uuml;gt.
     *     <P>
     *
     */
    public static PDate GetMontagVonWoche(int woche, int jahr) throws IllegalArgumentException {
        ...
        if (debug) {
            System.out.println("\nEntrypoint: PDate.GetMontagVonWoche(" + woche + ", " + jahr 
                    + ")");
            System.out.println("    maxwoche=" + maxwoche);
        }
        ...
        if (debug) {
            ...
            System.out.println("    pd.getWoche()=" + pd.getWoche());
        }
        if (pd.getWoche() == 1) {
