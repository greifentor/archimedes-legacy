...
public class DatabaseConnectionsXMLBuilder extends AbstractXMLBuilder {
    ...
    public XMLNode buildNodes(DatabaseConnectionProvider dcp, XMLNodeFactory xmlNodeFactory) {
        ...
        ArchimedesImportJDBCDataSourceRecord dsr =
                (ArchimedesImportJDBCDataSourceRecord) ((DataModel) dcp
                ).getImportDataSourceRecord();
        if (dsr == null) {
            dsr = new ArchimedesImportJDBCDataSourceRecord("IMPORT", "IMPORT", "usr", "", false,
                    false);
        }
        ...
        this.addAttribute(dcn, "description", (dsr.getDescription() == null ? "-"
                : dsr.getDescription()));
