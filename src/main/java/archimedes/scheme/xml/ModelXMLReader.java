...
/**
 * This class reads a XML file with Archimedes data model information and creates a new diagram
...
public class ModelXMLReader {
    ...
    public DataModel read(InputStream inputStream) {
        ...
        try {
            ...
            this.readAdditionalSQLStatements(dataModel, doc.getElementsByTagName(
                    "AdditionalSQLStatements"));
            this.readColors((NodeList) xpath.evaluate("/Diagram/Colors/*", doc,
                    XPathConstants.NODESET));
            this.readImportDataSources(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/DataSources/Import", doc, XPathConstants.NODESET));
            this.readDatabaseConnections(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/DataSources/DatabaseConnection", doc, XPathConstants.NODESET));
            this.readDiagrammParameters(dataModel, doc.getElementsByTagName("Parameters"));
            this.readDomains(dataModel, (NodeList) xpath.evaluate("/Diagram/Domains/Domain",
                    doc, XPathConstants.NODESET));
            this.readOptions(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/Options/Option", doc, XPathConstants.NODESET));
            this.readSequences(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/Sequences/Sequence", doc, XPathConstants.NODESET));
            this.readStereotypes(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/Stereotypes/Stereotype", doc, XPathConstants.NODESET));
            this.readViews(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/Views/View", doc, XPathConstants.NODESET));
            this.readTables(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/Tables/Table", doc, XPathConstants.NODESET));
            this.readComplexIndices(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/ComplexIndices/Index", doc, XPathConstants.NODESET));
            this.addTablesToViews(dataModel, (NodeList) xpath.evaluate(
                    "/Diagram/Views/View", doc, XPathConstants.NODESET));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void readDatabaseConnections(DataModel dataModel, NodeList nodes) {
        for (int i = 0, leni = nodes.getLength(); i < leni; i++) {
            ...
            dataModel.addDatabaseConnection(new DatabaseConnection(name, driver, url,
                    userName, DBExecMode.valueOf(dbExecMode), domains, notNull, references,
                    quote));
        }
    }
    ...
    private void readDiagrammParameters(DataModel dataModel, NodeList nodes) {
        Node node = nodes.item(0);
        dataModel.setAdditionalSQLScriptListener(this.getString(node,
                "additionalSQLScriptListener"));
        ...
        dataModel.setDBVersionDescriptionColumnName(this.getString(node,
                "dbVersionColumnDescriptionName"));
        dataModel.setDBVersionTableName(this.getString(node, "dbVersionTableName"));
        dataModel.setDBVersionVersionColumnName(this.getString(node,
                "dbVersionColumnVersionName"));
        ...
        dataModel.setRelationColorToExternalTables(this.getColor(node,
    ...
    private void readImportDataSources(DataModel dataModel, NodeList nodes) {
        for (int i = 0, leni = nodes.getLength(); i < leni; i++) {
            ...
            ArchimedesImportJDBCDataSourceRecord dsr =
                    new ArchimedesImportJDBCDataSourceRecord(driver, dbName, user, "",
                    domains, references);
            dsr.setName(name);
    ...
    private void readDomains(DataModel dataModel, NodeList nodes) {
        for (int i = 0, leni = nodes.getLength(); i < leni; i++) {
            ...
            DomainModel dm = this.objectFactory.createDomain(name, dataType, length,
                    decimalPlace);
    ...
    /*
    private void readModelOptions(DataModel dataModel, NodeList nodes) {
        for (int i = 0, leni = nodes.getLength(); i < leni; i++) {
            Node node = nodes.item(i);
            String name = this.getString(node, "name");
            String parameter = this.getString(node, "parameter");
            dataModel.addOption(new Option(name, parameter));
        }
    }
    */
        ...
        for (int i = 0, leni = nodes.getLength(); i < leni; i++) {
            ...
            for (int j = 0, lenj = children.getLength(); j < lenj; j++) {
                ...
                if ("Column".equals(child.getNodeName())) {
                    ...
                    ColumnModel c = this.objectFactory.createColumn(columnName,
                            dataModel.getDomainByName(domainName));
                    ...
                    c.setEditorReferenceWeight(ReferenceWeight.valueOf(this.getString(child,
                            "editorReferenceWeight")));
                    ...
                    c.setSequenceForKeyGeneration(dataModel.getSequenceByName(this.getString(
                            child, "sequenceForKeyGeneration")));
                    c.setSuppressForeignKeyConstraint(this.getBoolean(child,
                    ...
                    StereotypeModel st = dataModel.getStereotypeByName(this.getString(child,
                            "name"));
                    t.addStereotype(st);
                    ...
                    ((GUIObjectModel) t).setXY(v, this.getInteger(child, "x"), this.getInteger(
                            child, "y"));
                }
            }
            tms.add(t);
        ...
        for (int i = 0, leni = nodes.getLength(); i < leni; i++) {
            ...
            for (int j = 0, lenj = children.getLength(); j < lenj; j++) {
                ...
                if ("Column".equals(child.getNodeName())) {
                            ...
                            if (rt != null) {
                                ...
                                for (int l = 0, lenl = children1.getLength(); l < lenl; l++) {
                                    ...
                                    if ("View".equals(child1.getNodeName())) {
                                        ...
                                        corent.base.Direction d0 =
                                                corent.base.Direction.CreateDirection(
                                                this.getString(child1, "direction0"));
                                        corent.base.Direction d1 =
                                                corent.base.Direction.CreateDirection(
                                                this.getString(child1, "direction1"));
                                        ...
                                        if (c.getRelation() == null) {
                                            c.setRelation(this.objectFactory.createRelation(v,
                                                    c, d0, o0, rc, d1, o1));
                                        } else {
                                            ((GUIRelationModel) c.getRelation()
                                                    ).setDirection((GUIViewModel) v, 0, d0); 
                                            ((GUIRelationModel) c.getRelation()
                                                    ).setDirection((GUIViewModel) v, 1, d1);
                                            ((GUIRelationModel) c.getRelation()
                                                    ).setOffset((GUIViewModel) v, 0, o0);
                                            ((GUIRelationModel) c.getRelation()
                                                    ).setOffset((GUIViewModel) v, 1, o1);
                                        }
                                        NodeList children2 = child1.getChildNodes();
                                        for (int m = 0, lenm = children2.getLength(); m < lenm;
                                                m++) {
                    ...
                    table.addComboStringMember(this.objectFactory.createToStringContainer(c,
                            prefix, suffix));
                    ...
                    nr.setColumn(c);
                    nr.setDeleteConfirmationRequired(this.getBoolean(child,
                            "deleteConfirmationRequired"));
                    nr.setExtensible(this.getBoolean(child, "extensible"));
                    nr.setId(this.getInteger(child, "id"));
                    nr.setNReferencePanelType(NReferencePanelType.valueOf(this.getString(child,
                            "nReferencePanelType")));
                    ...
                    om.setOrderDirection(OrderClauseDirection.valueOf(this.getString(child,
                            "direction")));
                    ...
                    TableModel t = dataModel.getTableByName(tableName);
                    SelectionAttribute selectionAttribute = SelectionAttribute.valueOf(
                            this.getString(child, "selectionAttributeName"));
                    ColumnModel c = t.getColumnByName(columnName);
                    ...
                    String prefix = this.getString(child, "prefix");
                    String suffix = this.getString(child, "suffix");
                    table.addToStringMember(this.objectFactory.createToStringContainer(c,
                            prefix, suffix));
                }
            }
        }
    ...
    private int getInteger(Node node, String name) {
        ...
        try {
            ...
            throw new NumberFormatException("attribute value of '" + this.getPath(node) + "' "
                    + "is not a number: " + s);
        }
    }
