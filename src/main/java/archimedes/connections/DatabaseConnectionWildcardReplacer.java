...
public class DatabaseConnectionWildcardReplacer {
    ...
    public DatabaseConnection replace() {
        ...
        dc.setUrl(this.replaceWildcard(dc.getUrl(), "%DB_SERVER_NAME%",
                "archimedes.user.database.server.name"));
        dc.setUserName(this.replaceWildcard(dc.getUserName(), "%DB_USER_NAME%",
                "archimedes.user.database.user.name"));
        return dc;
    }
