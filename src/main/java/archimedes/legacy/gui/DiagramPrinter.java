...
import archimedes.legacy.print.CPJDiagramm;
import corent.djinn.DialogEditorDjinn;
import corent.files.Inifile;
import corent.print.CorePrintJob;
import corent.print.PrintJobConf;
...
public class DiagramPrinter {
	...
	private static final Logger log = Logger.getLogger(DiagramPrinter.class);

	/**
	 * Prints the family tree.
	 *
	 * @param frame               The frame which works as parent for the print
	 *                            dialog.
	 ...
	 * @param printFileName       A name for the file which is to print into in case
	 *                            of printing to a file.
	 *
	 * @changed OLI 23.05.2013 - Added.
	 */
	public void print(Frame frame, Inifile ini, DiagramComponentPanel comp, boolean toPDF, String standardPrinterName,
		...
		for (int i = 0, len = ps.length; i < len; i++) {
			...
			log.info(ps[i].getName());
		}
		log.info("");
		log.info("pages: " + component.getPrintPageCount());
			...
			public void doChanged(boolean saveOnly) {
				...
				try {
					...
					if (cpj.preview()) {
						cpj.print();
						if (pdfprint) {
							Process p = Runtime.getRuntime().exec("ps2pdf " + printfilename + ".ps");
							p.waitFor();
