...
import archimedes.legacy.gui.diagram.GUIObjectCreator;
import archimedes.legacy.model.gui.GUIDiagramModel;
import archimedes.legacy.model.gui.GUIViewModel;
import archimedes.legacy.transfer.CopyAndPasteController;
 ...
 * @changed OLI 18.05.2016 - Renamed to "DiagramComponentPanel" and extended by the logic for the
 *          "DiagramComponentPanelListener".
 */

public class DiagramComponentPanel<T extends Enum<?>> extends JPanel implements MouseListener, StatusBarOwner {
	...
	private List<DiagramComponentPanelListener> listeners = new LinkedList<DiagramComponentPanelListener>();
	 ...
	 * @changed OLI 22.05.2013 - Added.
	 */
	public DiagramComponentPanel(GUIViewModel vm, int width, int height, GUIDiagramModel diagram,
			GUIObjectCreator<T> guiObjectCreator, ComponentDiagramListener componentDiagramListener,
		...
		this.componentDiagram = new ComponentDiagramm<T>(vm, width, height, diagram, guiObjectCreator,
				componentDiagramListener, this, copyAndPasteController);
	...
	/**
	 * Adds the passed listener to the listeners which observe the component if not already added.
	 *
		...
		try {
			for (DiagramComponentPanelListener l : this.listeners) {
				try {
					...
					LOG.error("error occured while processing event for listener: " + l + ", event: " + event);
				}
			}
		} catch (Exception e) {
	...
	public void mouseClicked(MouseEvent e) {
		this.fireDiagramComponentPanelEvent(
				new DiagramComponentPanelEvent(DiagramComponentPanelEvent.Type.WARNINGS_CLICKED, e.getClickCount()));
	}
