...
public class DefaultCodeFactory implements CodeFactory {
	...
	private static final Logger log = Logger.getLogger(DefaultCodeFactory.class);
	...
	public boolean generate(String out) {
		...
		try {
			...
			log.info("writing " + path + dm.getApplicationName() + "/" + dm.getApplicationName() + "Core.java");
			...
			try {
				...
				log.info("directory " + path + dm.getBasePackageName().replace(".", "/") + " created");
			} catch (Exception e) {
			}
	...
	/**
	 * @changed OLI 26.03.2008 - Erweiterung um die Erzeugung einer Ressourcendatei
	 *          zur Tabelle.
	 * 
	 */
	protected void codeTable(TabellenModel tm, DiagrammModel dm, String out) throws Exception {
		...
		/*
		 * // und jetzt wird's geschrieben ... log.info("writing " + out + tm.getName()
		 * + ".java"); FileWriter fw = new FileWriter(out + tm.getName() + ".java",
		 * false); BufferedWriter writer = new BufferedWriter(fw); for (int i = 0, len =
		 * zeilen.size(); i < len; i++) { writer.write((String) zeilen.elementAt(i) +
	 ...
	 * @return Der Name des Java-Datentypen bzw. 'UNKNOWN', wenn kein Datentyp zur
	 *         Domain gebildet werden kann.
	 * 
	 ...
	 *          konfigurieren. Die Voreinstellung (bei undefinierter Property) ist
	 *          "corentx.dates".
	 */
	public static String GetType(DomainModel d) {
		...
		/*
		 * int type = d.getDatatype(); if (d.getName().equalsIgnoreCase("Boolean")) {
		 * type = Types.BOOLEAN; } if (d.getName().equalsIgnoreCase("PDate") ||
		 * d.getName().equalsIgnoreCase("PTime") ||
		 * d.getName().equalsIgnoreCase("PTimestamp") ||
		 * d.getName().equalsIgnoreCase("LongPTimestamp")) { return "corent.dates." +
		 * d.getName(); } String s = "UNKNOWN"; switch (type) { case Types.BIGINT: s =
		 * "long"; break; case Types.BIT: case Types.BOOLEAN: s = "boolean"; break; case
		 * Types.BINARY: case Types.LONGVARBINARY: case Types.VARBINARY: s = "byte[]";
		 * break; case Types.CHAR: case Types.LONGVARCHAR: case Types.VARCHAR: s =
		 * "String"; break; case Types.DATE: s = "java.sql.Date"; break; case
		 * Types.DECIMAL: case Types.DOUBLE: case Types.FLOAT: case Types.NUMERIC: s =
		 * "double"; break; case Types.INTEGER: case Types.SMALLINT: case Types.TINYINT:
		 * s = "int"; break; case Types.REAL: s = "float"; break; case Types.TIME: s =
		 * "java.sql.Time"; break; case Types.TIMESTAMP: s = "java.sql.Timestamp";
		 * break; } return s;
		 */
	}

	/**
	 * Liefert einen Standardwert im Stringformat f&uuml;r die Initialisierung eines
	 * zum DomainModel passenden Java-Datentypen.
	 * 
	 * @param d Das DomainModel, zu dem der Initializer generiert werden soll.
	 * @return Der Intialwert im Stringformat des Java-Datentypen bzw. 'UNKNOWN',
	 *         wenn kein Datentyp zur Domain gefunden werden kann.
	 */
	...
	/**
	 * Liefert einen Standardwert f&uuml;r die Initialisierung eines zum DomainModel
	 * passenden Java-Datentypen.
	 * 
	 * @param d Das DomainModel, zu dem der Initializer generiert werden soll.
	 ...
	 * @return Der Name der zum elementaren Java-Datentyp bzw. der &uuml;bergebene
	 *         Typname, falls zu dem Typnamen kein Wrapper gefunden werden kann.
	 */
	...
	/**
	 * Pr&uuml;ft, ob der der &uuml;bergebenen Domain zugeordnete Java-Datentyp ein
	 * Referenztyp ist.
	 * 
	 ...
	 * Tabelle.
	 * 
	 * @param res       Der Vector, an den die erzeugten Ressourcen angeh&auml;ngt
	 *                  werden sollen.
	 * @param classname Der Name, den die Klasse in der Applikation haben wird.
	 * @param tm        Das TabellenModel, zu dem die Ressourcen generiert werden
	 *                  sollen.
	 * 
	 * @changed OLI 28.03.2008 - Hinzugef&uuml;gt.
	 * 
