...
public class MetaDataReader {
	 ...
	 * @changed OLI 04.12.2015 - Added.
	 */
	public MetaDataModel read(JDBCDataSourceRecord connection, String schemeName, DBExecMode dbMode)
			throws SQLException {
		...
		if (dbMode == DBExecMode.POSTGRESQL) {
			MetaDataUniqueWithNullableConstraintReader uwncr = new MetaDataUniqueWithNullableConstraintReader();
	 ...
	 * @param dbMode    A DBMS mode which the meta data model is to compare with or <CODE>null</CODE> if there is
	...
	public MetaDataModel read(DataModel dataModel, DBExecMode dbMode) {
		...
		MetaDataModel m = new MetaDataModel(dataModel.getName(), this.getTemporaryPrefixIfExists(dataModel));
