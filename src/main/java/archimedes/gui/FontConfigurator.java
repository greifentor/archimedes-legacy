...
/**
 * This class configures the font for the application by an JavaScript and via the UIManager.
 * <BR>The JavaScript contains statements to fill a map (String, Font) which is processed by the
 * <CODE>FontConfigurator</CODE>. The JavaScript is could be placed in the users home folder or
 * in the home folder of the Archimedes installation. It has to be named as
 * ".archimedes-fonts.js". 
 *
 * @author O.Lieshoff
 *
    ...
    public void readFonts() throws IOException, ScriptException {
        ...
        for (Map.Entry<Object, Object> entry : javax.swing.UIManager.getDefaults().entrySet()) {
            ...
            // if (value instanceof FontUIResource) {
                ...
                // sv.add("m.put(\"" + key + "\", new FontUIResource(fontName, Font."
                //         + (style == Font.PLAIN ? "PLAIN" : (style == Font.BOLD ? "BOLD" :
                //         (style == Font.ITALIC ? "ITALIC" : "PLAIN"))) +  ", fontSize));");
                sv.add(key + " > " + value);
            // }
        }
        for (String se : sv) {
