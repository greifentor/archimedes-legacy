 ...
 * <P>&Uuml;ber die Property <I>archimedes.gui.ComponentDiagramm.HIT_TOLERANCE</I> kann die 
 * Entfernung beeinflu&szlig;t werden, in der ein Objekt von der Mouse-Position liegen 
 * mu&szlig;, um noch als getroffen zu gelten. Die Defaulteinstellung liegt bei 10.
 * <P>Die Property <I>archimedes.gui.ComponentDiagramm.SmartPositioner.mode</I> erm&ouml;glicht
 * die Beeinflussung der SmartPositioner-Position. &Uuml;ber <TT>CENTER</TT> wird der 
 * SmartPositioner in die Mitte der Tabelle gesetzt. Mit dem Schl&uuml;sselwort <TT>LEFTTOP</TT>
 * erscheint er an der linken, oberen Ecke der Tabelle. Weitere Modi sind nicht implementiert.
 *
 * @param <T> An enumeration which contains the identifiers for creating objects.
 *
 * @author O.Lieshoff
 *
 * @changed OLI 29.08.2007 - Anpassung der Methoden <TT>paint(Graphics)</TT> an die
 *          M&ouml;glichkeit, Tabellen &uuml;ber Stereotypen aus dem Diagramm auszublenden.
 *          Hierzu war auch eine Anpassung der bisherigen Methodik, Tabellen auszublenden,
 *          n&ouml;tig.
 * @changed OLI 18.12.2007 - Erweiterungen im Rahmen der Copy-&amp;-Paste-Funktion f&uuml;r 
 *          Tabellen.
 * @changed OLI 01.01.2008 - Erweiterung der Methode <TT>paint(Graphics, boolean, boolean)</TT>
 *          um den zweiten Boolean-Parameter, der anzeigt, ob die Methode zur Generierung einer
 *          Bildschirmanzeige oder eines Ausdrucks (bzw. Grafikexports) genutzt werden soll.
 * @changed OLI 16.12.2008 - Entstatifizierung des Zoomfaktors.
 * @changed OLI 20.12.2011 - Einf&uuml;gen der Bereinigung der komplexen Indices beim
 *         L&ouml;schen einer Tabelle.
 */
 
public class ComponentDiagramm<T extends Enum<?>> extends JComponent 
    ...
    public static int HIT_TOLERANCE = Integer.getInteger(
            "archimedes.gui.ComponentDiagramm.HIT_TOLERANCE", 10);
    /** Anzahl der Seiten pro Spalte. */
    public static int PAGESPERCOLUMN = 
            Integer.getInteger("archimedes.gui.ComponentDiagramm.PAGESPERCOLUMN", 3);
    /** Anzahl der Seiten pro Zeile. */
    public static int PAGESPERROW =
            Integer.getInteger("archimedes.gui.ComponentDiagramm.PAGESPERROW", 4);
    ...
    /* 
     * Diese Flagge ist gesetzt, wenn sich die Anzeigekomponente im Einf&uuml;gemodus f&uuml;r 
     * Tabelle befindet. 
     */
    private boolean insertmode = false;
    /* 
     * Wenn diese Flagge gesetzt ist, wird eine Tabelle mit Standardeinstellungen und -Spalten
     * generiert.
     */
    ...
    /* 
     * Wird dieser Wert gesetzt, so wird ein Raster auf die Arbeitsfl&auml;che gemalt. Der 
     * Abstand der Rasterpunkte entspricht dem gesetzten Wert. Ein Wert von <= 0 schaltet das 
     * Raster aus.
     */
    private int rasterwidth = 25;
    private int exposedRasterPointAnyXPointsHeight = Integer.getInteger(
            "archimedes.gui.ComponentDiagramm.exposed.raster.point.any.x.points.height", 5);
    private int exposedRasterPointAnyXPointsWidth = Integer.getInteger(
            "archimedes.gui.ComponentDiagramm.exposed.raster.point.any.x.points.width", 10);
    ...
    /**
     * Erzeugt eine neue Komponente zur Abbildung eines Diagramms anhand der &uuml;bergebenen 
     * Parameter.
     *
     ...
     * @param statusBarOwner A component which is to update if the status of the component is
     *         changed.
     *
     * @changed
     *     OLI 18.12.2007 - Erweiterung um die Funktionen zur Copy-&amp;-Paste-Funktion f&uuml;r
     *             Tabellen.<BR>
     *
     */
    public ComponentDiagramm(GUIViewModel vm, int width, int height,
            GUIDiagramModel diagram, GUIObjectCreator<T> guiObjectCrtr,
            ComponentDiagramListener<T> componentDiagramListener, StatusBarOwner statusBarOwner,
        ...
        this.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                ...
                if ((e.getButton() == MouseEvent.BUTTON3) && e.isControlDown()) {
                    ...
                    try {
                        ...
                        try {
                            TableModel[] tms = copyAndPasteController.transferableStringToTable(
                                    (DataModel) dm, s);
                            for (TableModel tm : tms) {
                                ((GUIObjectModel) tm).setXY(getView(), roundCoor(e.getX(),
                                        e.isShiftDown()), roundCoor(e.getY(), e.isShiftDown()));
                            }
                        } catch (Exception ex) {
                            ex.printStackTrace();
                        }
                ...
                if ((e.getButton() == MouseEvent.BUTTON3)) {
                    for (int i = 0; i < len; i++) {
                        if (v.elementAt(i) instanceof GUIRelationModel) {
                            ...
                            Point p = r.getPoint(getView(), roundCoor(convertReverse(e.getX()), 
                                    e.isShiftDown()), roundCoor(convertReverse(e.getY()), 
                                    e.isShiftDown()));
                ...
                if ((e.getClickCount() == 2) && (len > 0) && (v.elementAt(0) instanceof 
                        GUIObjectModel)) {
            ...
            @Override public void mousePressed(MouseEvent e) {
                if (insertmode) {
                    insertmode = false;
                    if (!e.isPopupTrigger()) {
                        GUIObjectModel om = guiObjectCreator.create(getView(), 
                                roundCoor(e.getX(), e.isShiftDown()), roundCoor(e.getY(), 
                                e.isShiftDown()), guiObjectToCreateIdentifier, isStandardMode()
                                );
                        dm.addGUIObject(om);
                        dm.raiseAltered();
                    }
            ...
            @Override public void mouseReleased(MouseEvent e) {
                ...
                if (dragging) {
                    ...
                    if (dragged instanceof GUIObjectModel) {
                        ...
                        t.setXY(getView(), roundCoor(convertReverse(e.getX()) - draggingOffsetX, 
                                e.isShiftDown()), roundCoor(convertReverse(e.getY()) - 
                                draggingOffsetY, e.isShiftDown()));
                ...
                if (!dragging) {
                    ...
                    if (len > 0) {
                        if (v.elementAt(0) instanceof GUIObjectModel) {
                            ...
                            Point p = r.getPoint(getView(), convertReverse(roundCoor(e.getX(), 
                                    e.isShiftDown())), convertReverse(roundCoor(e.getY(), 
                                    e.isShiftDown())));
                        ...
                        if ((len > 1) && (v.elementAt(1) instanceof GUIRelationModel)) { 
                            ...
                            if (r.isEndPoint(getView(), p)) {
                                dragged = new RelationEndPointContainer(r, p, 
                                        r.getObjectPointed(getView(), p));
                            } else {
                                dragged = new RelationPointContainer(r, p);
                            }
                        }
                    ...
                    if (dragged instanceof GUIObjectModel) {
                        ...
                        rpc.relation.computeOffset(getView(), rpc.object, roundCoor(e.getX(), 
                                e.isShiftDown()), roundCoor(e.getY(), e.isShiftDown()));
                        repaint();
                    } else if (dragged instanceof RelationPointContainer) {
                        RelationPointContainer rpc = (RelationPointContainer) dragged;
                        rpc.point.move(x, y);
                        /*
                        rpc.point.move(roundCoor(e.getX(), e.isShiftDown()), roundCoor(e.getY(),
                                e.isShiftDown()));
                                */
                        repaint();
                    }
    ...
    /** 
     * @return Eine Liste mit den ShapeContainern, deren Shapes durch die angegebenen 
     *     Koordinaten getroffen werden. 
     */ 
    ...
    public Vector hits(int x, int y, int xht, int yht) {
        ...
        for (int i = 0, len = this.shapes.size(); i < len; i++) {
            ...
            if ((sc.obj != null) && (sc.shape != null) && ((sc.shape.contains((double) x, 
                    (double) y) || sc.shape.intersects((double) x - xht / 2, (double) y - yht 
                    / 2, xht, yht)))) {
                v.addElement(sc.obj);
            }
        }
     ...
     *
     * @deprecated Hier hat sich der Fehlerteufel eingeschlichen. Richtig mu&szlig; es heissen:
     *     setRasterWidth(int).
     *
    ...
    /** 
     * @return <TT>true</TT>, wenn sich die Komponente im Einf&uuml;gemodus f&uuml;r Tabellen
     *     befindet, <TT>false</TT> sonst.
     */
     ...
     *
     * @changed
     *     OLI 16.12.2008 - Hinzugef&uuml;gt.
     *     <P>
     *
     */
    @Override public double getZoomFactor() {
    ...
    public Point getLeftUpper(int page) {
        ...
        case 1:
            break;
        case 2:
            x0 = ComponentDiagramm.DOTSPERPAGEX;
            break;
        case 3:
            x0 = ComponentDiagramm.DOTSPERPAGEX * 2;
            break;
        case 4:
            x0 = ComponentDiagramm.DOTSPERPAGEX * 3;
            break;
        case 5:
            y0 = ComponentDiagramm.DOTSPERPAGEY;
            break;
        case 6:
            x0 = ComponentDiagramm.DOTSPERPAGEX;
            y0 = ComponentDiagramm.DOTSPERPAGEY;
        ...
        case 8:
            ...
            break;
        case 9:
            y0 = ComponentDiagramm.DOTSPERPAGEY * 2;
            break;
        case 10:
            x0 = ComponentDiagramm.DOTSPERPAGEX;
            y0 = ComponentDiagramm.DOTSPERPAGEY * 2;
    ...
    public Point getPrintPageCountXY() {
        ...
        while (page <= MAXPAGECOUNT) {
            ...
            if (this.hits((int) p.getX() + DOTSPERPAGEX / 2, (int) p.getY() + DOTSPERPAGEY / 2, 
                    DOTSPERPAGEX, DOTSPERPAGEY).size() > 0) {
                xmax = (x > xmax ? x : xmax);
                ymax = (y > ymax ? y : ymax);
            }
            x++;
    ...
    /**
     * Setzt die Ansicht des Tabellenschemas auf die Tabelle mit dem angegebenen Namen. Ist eine
     * Tabelle mit dem &uuml;bergebenen Namen nicht bekannt, wird eine Exception geworfen.
     *
     * @param tn Der Name der Tabelle, auf die die Diagrammsicht gesetzt werden soll.
     * @throws NoSuchElementException wenn der Tabellenname in dem der Anzeige zugrundeliegenden
    ...
    /**
     *
     * @param printout Diese Flagg mu&szlig; gesetzt werden, wenn es sich bei dem Malvorgang
     *     um einen Ausdruck oder Grafikexport handelt.
     *
     * @changed
     *     OLI 29.08.2007 - Anpassung an die M&ouml;glichkeit, Tabellen &uuml;ber Stereotypen
     *             aus dem Diagramm auszublenden. Hierzu war auch eine Anpassung der bisherigen
     *             Methodik, Tabellen auszublenden, n&ouml;tig.<BR>
     *     OLI 01.01.2008 - Erweiterung um die Flagge <TT>printout</TT> in der Parameterliste
     *             der Methode. Hiermit wird angezeigt, ob es sich um einen Druck (bzw. 
     *             Grafikexport) oder eine Bildschirmausgabe handelt.<BR>
     *
     */
    public void paint(Graphics g, boolean raster, boolean printout) {
            ...
            for (int i = 0; i < this.getWidth(); i += this.rasterwidth) {
                for (int j = 0; j < this.getHeight(); j += this.rasterwidth) {
                    if ((j % (this.rasterwidth * this.exposedRasterPointAnyXPointsHeight) == 0)
                            && (i % (this.rasterwidth * this.exposedRasterPointAnyXPointsWidth)
                            == 0)) {
        ...
        int y = 30;
        g.setFont(new Font("sansserif", Font.BOLD, (int) ((double) 
                this.dm.getFontSizeDiagramHeadline() * this.getZoomFactor())));
        g.drawString(dm.getName(), convert(20), convert(y));
        g.setFont(new Font("sansserif", Font.BOLD, (int) ((double) 
                this.dm.getFontSizeSubtitles() * this.getZoomFactor())));
        y += g.getFontMetrics().getHeight() + 5;
        g.drawString(this.dm.getAuthor(), convert(20), convert(y));
        y += g.getFontMetrics().getHeight() + 5;
        g.drawString("Version " + this.dm.getVersion() + " - " + this.dm.getDateOfCreation(),
                convert(20), convert(y));
        y += g.getFontMetrics().getHeight() + 5;
        ...
        if (this.dm != null) {
            for (GUIObjectModel tm : this.dm.getGUIObjects()) {
                ...
                if (showTable) { 
                    this.shapes.addElement(tm.paintObject(this, this.getView(), g, (printout 
                            ? PaintMode.PRINTER : PaintMode.EDITOR)));
                    if (this.dragged == tm) {
                        ...
                        String mode = System.getProperty(
                                "archimedes.gui.ComponentDiagramm.SmartPositioner.mode", 
                                "CENTER").toUpperCase();
                        if (mode.equals("CENTER")) {
                            ...
                            g.drawOval(convert(xm-sm), convert(ym-sm), convert(sm*2), convert(
                                    sm*2));
                        } else if (mode.equals("LEFTTOP")) {
                            g.drawLine(convert(tmx-sm), convert(tmy), convert(tmx+sm), convert(
                                    tmy));
                            g.drawLine(convert(tmx), convert(tmy-sm), convert(tmx), convert(
                                    tmy+sm));
                            g.drawOval(convert(tmx-sm), convert(tmy-sm), convert(sm*2), convert(
                                    sm*2));
                        }
                        g.setColor(Color.black);
                    }
            ...
            for (GUIObjectModel tm : this.dm.getGUIObjects()) {
                ...
                if (showTable) { 
                    g.setColor(Color.black);
                    for(ShapeContainer sc : tm.paintRelations(this, this.getView(), g, (printout
                            ? PaintMode.PRINTER : PaintMode.EDITOR))) {
    ...
    @Override public int convert(int k) {
        return (int) ((double) k * this.getZoomFactor());
    }
    
    /** Rechnet die &uuml;bergebene Koordinate anhand des Zoomfaktors um. */
    @Override public int convert(double k) {
        return (int) ((double) k * (double) this.getZoomFactor());
    }
    
    @Override public int convertReverse(int k) {
        return (int) ((double) k / (double) this.getZoomFactor());
    }
    ...
    public void updateLabelPosition(int x, int y) {
        ...
        if ((this.getDragged() == null) && (this.getClicked() != null)) {
            if (this.getClicked() instanceof GUIObjectModel) {
                clicked = this.statusBarOwner.getRenderer().renderObject(
                        (GUIObjectModel) this.getClicked()); 
            } else if (this.getClicked() instanceof GUIRelationModel) {
                clicked = this.statusBarOwner.getRenderer().renderRelation(
                        (GUIRelationModel) this.getClicked()); 
                /*
                Relation r = (Relation) this.getClicked();
                TabellenspaltenModel ts0 = r.getgetReferencer();
                ...
                clicked = (t0 != null ? t0.getName() : "<null>") + " -> " + (t1 != null ? 
                        t1.getName() : "<null>");
                        */ 
            }
        }
        this.statusBarOwner.updateStatusMessage("(" + x + "," + y + ")" + (this.getDragged() 
                != null ? "  - " + this.getDragged().toString() : (clicked != null ? "  - "
                + clicked : "")) + (this.isInsertMode() ? StrUtil.FromHTML(" EINF&Uuml;GEN!")
                : ""));
    }
