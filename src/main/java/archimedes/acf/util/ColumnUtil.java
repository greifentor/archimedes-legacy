...
public class ColumnUtil extends ParameterUtilOnwer {
    ...
    /**
     * Returns a list of columns from the passed array which contains only editor member columns
     * which are assorted by their editor positions.
     *
     * @param columns The array of columns where editor member columns are to separate from.
     * @return A list of columns from the passed array which contains only editor member columns
     *         which are assorted by their editor positions.
     *
    ...
    public ColumnModel[] getEditorColumnsOrderedByPositions(ColumnModel[] columns) {
        ...
        Hashtable<Integer, SortedVector<Integer>> hpos = new Hashtable<Integer,
                SortedVector<Integer>>();
        ...
        for (Integer tab : hpos.keySet()) {
            ...
            for (Integer i : pos) {
                for (ColumnModel c : l0) {
                    if ((c.getEditorPosition() == i.intValue())
                            && (tab == c.getPanel().getPanelNumber())) {
                        l.add(c);
                    }
                }
            }
    ...
    /**
     * Returns the order column for the passed table model or <CODE>null</CODE> if none is
     * defined.
     *
     * @param tm The table model which is to check for a order column.
     * @return The order column for the passed table model or <CODE>null</CODE> if none is
     *         defined.
     *
     * @changed OLI 03.06.2015 - Added.
     */
