...
public class CodeGeneratorListConfigurationDialog extends JDialogWithInifile implements ActionListener {
	 ...
	 * @changed OLI 16.10.2013 - Added.
	 */
	public CodeGeneratorListConfigurationDialog(GUIBundle guiBundle, CodeGenerator[] codeGenerators,
			IndividualPreferences ip, TableModel[] tables, String configFilePath, String codeFactoryVersion) {
		...
		this.setTitle(this.guiBundle.getResourceText("code.generators.configuration.title").replace("$VERSION$",
				codeFactoryVersion));
		this.setLayout(new BorderLayout(this.guiBundle.getHGap(), this.guiBundle.getVGap()));
		JPanel main = new JPanel(new BorderLayout(this.guiBundle.getHGap(), this.guiBundle.getVGap()));
		main.setBorder(new EmptyBorder(this.guiBundle.getVGap(), this.guiBundle.getHGap(), this.guiBundle.getVGap(),
	...
	private JPanel createPathLabelPanel(String basePath) {
		JLabel label = new JLabel(
				this.guiBundle.getResourceText("code.generators.configuration" + ".label.path.label"));
	...
	private JButton createButton(String name) {
		JButton button = new JButton(
				this.guiBundle.getResourceText("code.generators." + "configuration.button." + name + ".label"));
	...
	private JPanel createTablePanel() {
		JPanel p = new JPanel(new GridLayout(1, 2, this.guiBundle.getHGap(), this.guiBundle.getVGap()));
	...
	private JPanel createTablesTablePanel() {
		JTable tableTables = new JTableWithInifile(new TablesTableModel(this.tables, this.guiBundle),
				this.guiBundle.getInifile(), "TablesConfigurationTable");
		JPanel panelTables = new JPanel(new BorderLayout(this.guiBundle.getHGap(), this.guiBundle.getVGap()));
	...
	private JPanel createGeneratorsTablePanel() {
		JTable tableGenerators = new JTableWithInifile(
				new CodeGeneratorListTableModel(this.codeGenerators, this.guiBundle), this.guiBundle.getInifile(),
				"CodeGeneratorConfigurationTable");
		tableGenerators.setDefaultRenderer(String.class, new TableGeneratorsCellRenderer(this.codeGenerators));
		JPanel panelGenerators = new JPanel(new BorderLayout(this.guiBundle.getHGap(), this.guiBundle.getVGap()));
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.buttonCancel) {
			...
			this.repaint();
		} else if ((e.getSource() == this.buttonSelectTablesAll) || (e.getSource() == this.buttonSelectTablesNone)) {
