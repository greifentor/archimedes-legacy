...
public class ModelCheckerTableNameDoesNotContainSpecialCharacters implements ModelChecker {
	...
	public static final String RES_MODEL_CHECKER_COLUMN_WITHOUT_A_DOMAIN_LABEL = "ModelChecker.TableNameContainsASpecialCharacter.label";
			...
			for (int i = 0, leni = t.getName().length(); i < leni; i++) {
				...
				if ((c <= '/') || (":;<=>?@[\\]^`{|}~".indexOf(c) > -1)) {
					l.add(new ModelCheckerMessage(ModelCheckerMessage.Level.ERROR, this.guiBundle.getResourceText(
							RES_MODEL_CHECKER_COLUMN_WITHOUT_A_DOMAIN_LABEL, t.getName(), (int) c, i), t));
				}
			}
		}
