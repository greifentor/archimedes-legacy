...
import archimedes.acf.DoubleClassesChecker;
...
public class GenerationProcessReport {
	...
	private Map<TableModel, CodeGeneratorReport> generatorReports = new Hashtable<TableModel, CodeGeneratorReport>();
	 ...
	 * @changed OLI 28.11.2013 - Added.
	 */
	public void addBlockedByMissingInclusionMark(TableModel table, CodeGeneratorReportTableEntry entry) {
		this.prepareFor(table);
	...
	public String toHTML() {
		...
		String s = "<HTML>\n";
		s += "    <FONT FACE=\"ARIAL\">\n";
		s += "    <H1>" + this.guiBundle.getResourceText("generation.process.report.title") + "</H1>\n";
		s += "    <HR>\n";
		s += "    <B><FONT SIZE=+1>" + this.guiBundle.getResourceText("generation.process.report.state.label") + " "
				+ (this.success
						? this.createColorTag(SUCCESS_COLOR)
								+ this.guiBundle.getResourceText("generation.process.report.success.label") + "</FONT>"
						: this.createColorTag(FAULT_COLOR)
								+ this.guiBundle.getResourceText("generation.process.report.failed.label") + "</FONT>")
				+ "</FONT>";
		s += "</FONT></B><BR>\n";
		s += "    " + this.guiBundle.getResourceText("generation.process.report.version.label") + " " + this.version
				+ "<BR>\n";
		s += "    " + this.guiBundle.getResourceText("generation.process.report.finished.label") + " "
				+ new PTimestamp() + "\n";
		s += "    <HR>\n";
		if (this.generatedFileNames.size() > 0) {
			s += "    <H2>" + this.guiBundle.getResourceText("generation.process.report.unusedFileNames.title")
					+ "</H2>\n";
			s += "    <P>" + this.guiBundle.getResourceText("generation.process.report.unusedFileNames.number.label",
		...
		if (this.logLines.size() > 0) {
			s += "    <H2>" + this.guiBundle.getResourceText("generation.process.report.log.title") + "</H2>\n";
			s += "    <PRE>\n";
		...
		if (writtenFileNames.length > 0) {
			s += "    <H2>" + this.guiBundle.getResourceText("generation.process.report.written.file.names.title")
					+ "</H2>\n";
			s += "    <UL>\n";
		...
		s += "    <H2>" + this.guiBundle.getResourceText("generation.process.report.tables.title") + "</H2>\n";
		s += "    <UL>\n";
		for (TableModel table : this.sortTables(this.generatorReports.keySet())) {
			...
			s += "          <UL>\n";
			s += "              " + this.createReportForState("WRITTEN", cgr.getWritten(), WARN_COLOR);
			s += "              " + this.createReportForState("BLOCKED_BY_EXCLUSION", cgr.getBlockedByExclusion());
			s += "              " + this.createReportForState("BLOCKED_BY_MISSING_INCLUSION_" + "MARK",
					cgr.getBlockedByMissingInclusionMark());
			s += "              "
					+ this.createReportForState("DEPRECATED_SOURCE_CODE_DETECTED", cgr.getDeprecatedSourceCode());
			s += "              " + this.createReportForState("UNCHANGED", cgr.getUnchanged());
			s += "              " + this.createReportForState("UNCHANGED_BY_TAG", cgr.getUnchangedByTag());
			s += "          </UL>\n";
			deprecatedFilesDetected = deprecatedFilesDetected || (cgr.getDeprecatedSourceCode().length > 0);
		}
		s += "    </UL>\n";
		if ((this.doubleClassesChecker != null) && (this.doubleClassesChecker.hasWarnings())) {
			s += "    <HR>\n";
			s += "    <H2>" + this.guiBundle.getResourceText("generation.process.double.files.title") + "</H2>\n";
			s += "    <UL>\n";
		...
		if (deprecatedFilesDetected) {
			s += "    <HR>\n";
			s += "    <H2>" + this.guiBundle.getResourceText("generation.process.deprecated.files.detected.title")
					+ "</H2>\n";
			s += "    <UL>\n";
	...
	private String createReportForState(String stateName, CodeGeneratorReportTableEntry[] entries) {
	...
	private String createReportForState(String stateName, CodeGeneratorReportTableEntry[] entries,
		...
		if (entries.length > 0) {
			s += "          <LI><I>" + this.createColorTag(highLightColor)
					+ this.guiBundle.getResourceText("generation.process.report.state." + stateName + ".label") + ": "
					+ (highLightColor != null ? "</FONT></B>" : "") + "</I>\n";
	...
	private String getCodeGeneratorName(CodeGenerator codeGenerator) {
		return this.guiBundle.getResourceText(
				"code.generators.configuration.name." + codeGenerator.getClass().getSimpleName() + ".title");
