...
import archimedes.grammar.cfk.ComplexFKParser.ColumnNameListContext;
import archimedes.grammar.cfk.ComplexFKParser.ComplexForeignKeyExpressionContext;
...
public class ComplexForeignKeyFactory {
	...
	public ComplexForeignKeyModel[] create(TableModel table) {
		...
		if ((table.getComplexForeignKeyDefinition() != null) && !table.getComplexForeignKeyDefinition().isEmpty()) {
			...
			for (int i = 0, leni = tree.getChildCount(); i < leni; i++) {
				...
				if (prc instanceof ComplexForeignKeyExpressionContext) {
					l.add(this.parseComplexForeignKeyExpression((ComplexForeignKeyExpressionContext) prc,
							table.getName()));
				}
			}
		}
	...
	private ComplexForeignKeyModel parseComplexForeignKeyExpression(ComplexForeignKeyExpressionContext ctx,
			String tableName) {
		return new ComplexForeignKeyModel(tableName, this.parseColumnNames(ctx.columnNameList(0)),
				ctx.Identifier().getText(), this.parseColumnNames(ctx.columnNameList(1)));
	}
...
class ComplexForeignKeyErrorListener implements ANTLRErrorListener {
	...
	public void syntaxError(Recognizer<?, ?> recognizer, Object offendingSymbol, int line, int charPositionInLine,
		...
		StringBuffer sb = new StringBuffer("Syntax error at line ").append(line).append(":").append(charPositionInLine)
				.append(" - ").append(msg);
