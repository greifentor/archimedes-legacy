...
public class MetaDataUniqueWithNullableConstraintReader {
    ...
    /**
     * Adds the unique constraints with a nullable field meta data of the data model of the
     * passed JDBC connection.
     *
     * @param model The meta model which the read unique constraints should be added to.
     * @param connection The connection whose unique constraint information from meta data
     *         should be read.
     * @param excludedTableNames Names of tables whose unique constraints should be excluded
     *         from meta data read.
     ...
     * @changed OLI 17.02.2016 - Added.
     */
    public void addUniqueWithNullableConstraints(MetaDataModel model,
            JDBCDataSourceRecord connection, String[] excludedTableNames, DBExecMode dbMode
            ) throws SQLException {
    ...
    private void getUniqueWithNullableMetaData(MetaDataModel model, Connection c,
        ...
        ResultSet rs = DBExec.Query(c, "SELECT n.nspname scheme_name, t.relname table_name, "
                ...
                + "WHERE t.relname LIKE '%' AND indisprimary = 'f' AND n.nspname = '"
                + model.getSchemeName() + "'\n"
        ...
        while (rs.next()) {
            ...
            if (uid == null) {
                ...
                uid.table = table;
                cond = cond.replace("IS NOT NULL",  "").replace("IS NULL", "").replace("\"", ""
                        ).trim();
        ...
        for (String ixn : m.keySet()) {
            ...
            uid.table.addUniqueConstraint(new MetaDataUniqueWithNullableConstraint(ixn,
                    uid.table, uid.nullable, uid.columns.toArray(new MetaDataColumn[0])));
        }
        System.out.println();
    }
