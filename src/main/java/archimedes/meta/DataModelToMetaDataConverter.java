...
public class DataModelToMetaDataConverter {
    ...
    private MetaDataComplexForeignKeyFactory metaDataComplexForeignKeyFactory =
            new MetaDataComplexForeignKeyFactory();
    private NameGenerator nameGenerator = null;

    /**
     * Creates a new data model to meta data converter which the passed parameters.
     *
     * @param dbMode The type of the DBMS which the meta data model is to create for.
     *
    ...
    public void addComplexIndices(MetaDataModel model, IndexMetaData[] complexIndices) {
        for (IndexMetaData i : complexIndices) {
            ...
            MetaDataIndex mdi = new MetaDataIndex(i.getName(), mdt, cols.toArray(
                    new MetaDataColumn[0]));
            mdt.addIndex(mdi);
        }
    }
    ...
    public void addSequences(MetaDataModel model, SequenceModel[] sequences) {
        for (SequenceModel s : sequences) {
            model.addSequence(new MetaDataSequence(s.getName(), s.getStartValue(),
                    s.getIncrement()));
        }
    }

    /**
     * Changes the table information of the data model to those of the meta data model and adds
     * the table meta data to the meta data model.
     *
     * @param model The meta data model which the table information are to add.
    ...
    public void addTables(MetaDataModel model, TableModel[] tables) {
        for (TableModel t : tables) {
            ...
            for (ColumnModel c : t.getColumns()) {
                ...
                MetaDataColumn mdc = new MetaDataColumn(mdt, c.getName(), d.getName(),
                        DBType.Convert(d.getDataType()), d.getLength(), d.getDecimalPlace(),
                        c.isPrimaryKey(), c.isNotNull(), c.getDefaultValue(), c.isDeprecated());
                mdc.setUnique(c.isUnique());
                mdc.setIndexed(c.hasIndex());
                mdt.addColumn(mdc);
                if (c.isIndexSearchMember()) {
                    mdt.addIndex(new MetaDataIndex(this.nameGenerator.getIndexName(t.getName(),
                            c.getName()), mdt, mdc));
                }
            }
            model.addTable(mdt);
            ...
            for (ColumnModel c : t.getColumns()) {
                ...
                if ((c.getReferencedColumn() != null) && !c.isSuppressForeignKeyConstraint()) {
                    ...
                    mdc.addForeignKeyConstraint(new MetaDataForeignKeyConstraint(
                            this.nameGenerator.getForeignKeyName(t.getName(), c.getName()),
                            rmdc));
                }
            }
        }
        for (TableModel t : tables) {
            MetaDataTable mdt = this.getTable(model, t);
            if ((t.getComplexForeignKeyDefinition() != null)
                    && (t.getComplexForeignKeyDefinition().length() > 0)) {
                this.addComplexForeignKeys(model, mdt, t);
            }
        }
        for (TableModel t : tables) {
            MetaDataTable mdt = this.getTable(model, t);
            if ((t.getComplexUniqueSpecification() != null)
                    && (t.getComplexUniqueSpecification().length() > 0)) {
    ...
    private void addComplexForeignKeys(MetaDataModel model, MetaDataTable mdt, TableModel t) {
        for (ComplexForeignKeyModel cfk : this.complexForeignKeyfactory.create(t)) {
            String fkName = this.nameGenerator.getForeignKeyName(cfk.getTableName(),
                    cfk.getColumnNames());
            mdt.addComplexForeignKey(this.metaDataComplexForeignKeyFactory.create(fkName, 
                    this.getTable(model, cfk.getTableName()), this.getTable(model,
                    cfk.getTargetTableName()), cfk));
        }
    }
    ...
    private void addComplexUniques(MetaDataTable mdt, String uniqueSpecification) {
        if (uniqueSpecification.contains("?")) {
            for (String us : Str.splitToList(uniqueSpecification, "|")) {
                ...
                mdt.addUniqueConstraint(new MetaDataUniqueWithNullableConstraint(
                        mdt.getName() + "_unique_index", mdt, nullable,
                        columns.toArray(new MetaDataColumn[0])));
                columns.remove(nullable);
                mdt.addUniqueConstraint(new MetaDataUniqueWithNullableConstraint(
                        mdt.getName() + "_unique_index_nullable", mdt, nullable,
                        columns.toArray(new MetaDataColumn[0])));
            }
        } else {
            for (String us : Str.splitToList(uniqueSpecification, "|")) {
                ...
                mdt.addUniqueConstraint(new MetaDataUniqueConstraint(
                        this.nameGenerator.getUniqueName(mdt.getName(), columnNames), mdt,
                        columns.toArray(new MetaDataColumn[0])));
                System.out.println(this.nameGenerator.getUniqueName(mdt.getName(), columnNames)
                        );
            }
        }
    }
