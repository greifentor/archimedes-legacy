...
public class MetaDataIndexReader {
     ...
     * @param excludedTableNames Names of tables whose indices should be excluded from meta data
     *         read.
     ...
     * @changed OLI 07.12.2015 - Added.
     */
    public void addIndices(MetaDataModel model, JDBCDataSourceRecord connection,
        ...
        for (MetaDataTable table : model.getTables()) {
            ResultSet rs = c.getMetaData().getIndexInfo(null, model.getSchemeName(),
        ...
        for (SimpleIndexMetaData imd : m.values()) {
            ...
            if (imd.getColumns().length == 1) {
                ...
                t.addIndex(new MetaDataIndex(imd.getName(), t,
                        converter.convertToMetaDataColumn(t, imd.getColumns())));
            }
        }
    }
