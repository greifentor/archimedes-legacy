...
/**
 * A comparator which is able to compare two meta data models and create a list of changes which
 * are necessary to equalize the two meta data model.
 *
 * @author O.Lieshoff
 *
...
public class MetaDataModelComparator {
    ...
    /**
     * Adds the passed additional script fragment (in string format) which is added to the
     * result of any compare operation.
     *
    ...
    /**
     * Compares the two models and returns a list of changes which have to be made to make
     * second passed model equal to the first one. These change operations have to be executed
     * to the second passed model.
     *
     ...
     * @return A list of change operations which have to be executed on the copy to make it
     *         equal to the model.
     *
     * @changed OLI 11.12.2015 - Added.
     */
    ...
    public void compareSequences(MetaDataModel model, MetaDataModel copy,
    ...
    public void compareTables(MetaDataModel model, MetaDataModel copy,
            List<AbstractChangeOperation> r) {
        for (MetaDataTable mdt : model.getTables()) {
            ...
            if (mdtc == null) {
                ...
                for (MetaDataColumn mdc : mdt.getColumns()) {
                    this.addColumnConstraints(mdt, mdc, r,
                            ScriptSectionType.ADD_CONSTRAINTS_FOR_NEW_TABLES);
                }
    ...
    private void addColumn(MetaDataTable table, MetaDataColumn column,
            List<AbstractChangeOperation> r) {
        if (table.isDeprecated() || column.isDeprecated()) {
            return;
        }
        r.add(new AddColumn(table, column));
        this.addColumnConstraints(table, column, r,
                ScriptSectionType.ADD_COLUMNS_AND_DROP_CONSTRAINTS, true);
    }

    private void addColumnConstraints(MetaDataTable table, MetaDataColumn column,
            List<AbstractChangeOperation> r, ScriptSectionType section) {
        this.addColumnConstraints(table, column, r, section, false);
    }

    private void addColumnConstraints(MetaDataTable table, MetaDataColumn column,
            List<AbstractChangeOperation> r, ScriptSectionType section, boolean noPrimaryKeys) {
        // if (columns.length == 1) {
            ...
            if (column.isPrimaryKey() && !noPrimaryKeys) {
                r.add(new AlterColumnAddConstraint(table, table.getPrimaryKeyMembers()[0],
                        ColumnConstraintType.PRIMARY_KEY, ScriptSectionType.UPDATE_CONSTRAINTS));
            }
    ...
    private void checkColumnType(MetaDataTable table, MetaDataColumn model,
            MetaDataColumn copy, List<AbstractChangeOperation> r) {
        if (model.getTypeName().equalsIgnoreCase("boolean")
                && model.getTypeName().equalsIgnoreCase(copy.getTypeName())) {
            return;
        }
        if (!model.getSQLType(DBExecMode.POSTGRESQL).equals(copy.getSQLType(
                DBExecMode.POSTGRESQL))) {
            r.add(new AlterColumnDataType(table, model, ScriptSectionType.ALTER_COLUMNS));
        }
    }

    private void checkDefaultValue(MetaDataTable table, MetaDataColumn model,
            MetaDataColumn copy, List<AbstractChangeOperation> r) {
        if (!Utl.equals(model.getDefaultValue(), copy.getDefaultValue())) {
            if (model.getDefaultValue() == null) {
                ...
                r.add(new AlterColumnSetDefaultValue(table, model,
                        ScriptSectionType.ALTER_COLUMNS));
            }
        }
    }

    private void checkColumnIndices(MetaDataTable table, MetaDataColumn model,
            MetaDataColumn copy, List<AbstractChangeOperation> r) {
        if (model.isIndexed() != copy.isIndexed()) {
            if (model.isIndexed()) {
                r.add(new CreateIndex(new MetaDataIndex(this.nameGenerator.getIndexName(
                        table.getName(), model.getName()), table, model),
                        ScriptSectionType.UPDATE_CONSTRAINTS));
            } else {
                r.add(new DropIndex(new MetaDataIndex(this.nameGenerator.getIndexName(
                        table.getName(), model.getName()), table, copy),
                        ScriptSectionType.ADD_COLUMNS_AND_DROP_CONSTRAINTS));
            }
        }
    }

    private void checkColumnNotNullConstraint(MetaDataTable table, MetaDataColumn model,
            MetaDataColumn copy, List<AbstractChangeOperation> r) {
    ...
    private void checkColumnUniqueConstraint(MetaDataTable table, MetaDataColumn model,
            MetaDataColumn copy, List<AbstractChangeOperation> r) {
    ...
    private void checkForeignKeyConstraints(MetaDataTable table, MetaDataColumn model,
            MetaDataColumn copy, List<AbstractChangeOperation> r) {
        for (MetaDataForeignKeyConstraint fkc : model.getForeignKeyConstraints()) {
            MetaDataForeignKeyConstraint fkcc = copy.getForeignKeyConstraint(
                    fkc.getReferencedColumn());
            if (fkcc != null) {
        ...
        for (MetaDataForeignKeyConstraint fkc : copy.getForeignKeyConstraints()) {
            MetaDataForeignKeyConstraint fkcc = model.getForeignKeyConstraint(
                    fkc.getReferencedColumn());
            if (fkcc == null) {
    ...
    private void compareUniqueConstraints(MetaDataTable model, MetaDataTable copy,
            List<AbstractChangeOperation> r) {
        for (MetaDataUniqueConstraint uc : model.getUniqueConstraints()) {
            if (uc.getColumns().length > 0) {
    ...
    private void compareIndices(MetaDataTable model, MetaDataTable copy,
            List<AbstractChangeOperation> r) {
        for (MetaDataIndex i : model.getIndices()) {
            if (i.getColumnNames().length > 0) {
    ...
    private void comparePrimaryKeys(MetaDataTable model, MetaDataTable copy,
            List<AbstractChangeOperation> r) {
    ...
    private String getCommaSeparatedPKs(MetaDataTable t) {
        return this.metaUtil.getCommaSeparated(this.metaUtil.getNames(t.getPrimaryKeyMembers()),
                false);
    }
