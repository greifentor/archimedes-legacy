...
public class SQLScriptGeneratorFactory {
     ...
     * @changed OLI 14.12.2015 - Added.
     */
    public SQLScriptGeneratorFactory(String quotes, boolean useDomains,
    ...
    public SQLScriptGenerator getSQLScriptGenerator(DBExecMode dbMode)
        ...
        if (dbMode == DBExecMode.POSTGRESQL) {
            return new PostgreSQLScriptGenerator(dbMode, this.quotes, this.useDomains,
                    this.foreignKeyConstraints, this.notNullConstraints, this.nameGenerator,
                    this.scriptHeaderBuilder);
        } else if (dbMode == DBExecMode.MYSQL) {
            return new StandardSQLScriptGenerator(dbMode, this.quotes, this.useDomains,
                    this.foreignKeyConstraints, this.notNullConstraints, this.nameGenerator,
                    this.scriptHeaderBuilder);
