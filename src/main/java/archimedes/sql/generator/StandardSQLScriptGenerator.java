...
import archimedes.meta.MetaDataUniqueWithNullableConstraint;
import archimedes.meta.MetaUtil;
import archimedes.meta.chops.AbstractChangeOperation;
import archimedes.meta.chops.ScriptSectionType;
import archimedes.meta.chops.addscripts.AddAdditionalScript;
import archimedes.meta.chops.columns.AddColumn;
import archimedes.meta.chops.columns.AlterColumnAddConstraint;
import archimedes.meta.chops.columns.AlterColumnDataType;
import archimedes.meta.chops.columns.AlterColumnDropConstraint;
import archimedes.meta.chops.columns.AlterColumnDropDefault;
import archimedes.meta.chops.columns.AlterColumnSetDefaultValue;
import archimedes.meta.chops.columns.ColumnConstraintType;
import archimedes.meta.chops.columns.DropColumn;
import archimedes.meta.chops.foreignkeys.AddForeignKeyConstraint;
import archimedes.meta.chops.foreignkeys.AddMultipleForeignKeyConstraint;
import archimedes.meta.chops.foreignkeys.DropForeignKeyConstraint;
import archimedes.meta.chops.foreignkeys.DropMultipleForeignKeyConstraint;
import archimedes.meta.chops.indices.CreateIndex;
import archimedes.meta.chops.indices.DropIndex;
import archimedes.meta.chops.primarykeys.AddPrimaryKeyConstraint;
import archimedes.meta.chops.primarykeys.DropPrimaryKeyConstraint;
import archimedes.meta.chops.sequences.AlterSequence;
import archimedes.meta.chops.sequences.CreateSequence;
import archimedes.meta.chops.sequences.DropSequence;
import archimedes.meta.chops.tables.CreateTable;
import archimedes.meta.chops.tables.DropTable;
import archimedes.meta.chops.uniques.AddComplexUniqueConstraint;
import archimedes.meta.chops.uniques.DropComplexUniqueConstraint;
...
/**
 * A SQL script generator for "Standard" SQL. It can be used as base for other
 * SQL script generator implementations.
 *
 * @author O.Lieshoff
 *
...
public class StandardSQLScriptGenerator implements SQLScriptGenerator {
	...
	private static final Logger log = Logger.getLogger(StandardSQLScriptGenerator.class);
	...
	/**
	 * Creates a new SQL script generator for standard SQL with the passed
	 * parameters.
	 *
	 * @param dbMode                The mode of the DBMS which the SQL script is to
	 *                              create for.
	 * @param quotes                The quotes for object identifiers (like table
	 *                              names).
	 * @param useDomains            Set this flag if the database should work with
	 *                              domains.
	 * @param foreignKeyConstraints Set this flag if the foreign key constraints
	 *                              should be set.
	 * @param notNullConstraints    Set this flag if the not null constraints should
	 *                              be set.
	 ...
	 */
	@Override
	public String generate(AbstractChangeOperation[] changesArr) throws IllegalArgumentException {
		...
		for (AbstractChangeOperation change : changes) {
			...
			if ((s != null) && (s.length() > 0)) {
				if (section != change.getSection()) {
					...
					sql += "\n\n" + Str.pumpUp("/** SECTION: " + section.name() + " ", "*", 69, Direction.RIGHT)
							+ "/\n\n";
				}
	...
	protected String getSQLStatementForChangeOperation(AbstractChangeOperation change) {
		if (change instanceof AddColumn) {
			...
			return this.transferAddAdditionalScriptToSQLStatement((AddAdditionalScript) change);
		} else if (change instanceof AddComplexUniqueConstraint) {
			return this.transferAddComplexUniqueConstraintToSQLStatement((AddComplexUniqueConstraint) change);
		} else if (change instanceof AddForeignKeyConstraint) {
			return this.transferAddForeignKeyConstraintToSQLStatement((AddForeignKeyConstraint) change);
		} else if (change instanceof AddMultipleForeignKeyConstraint) {
			return this.transferAddMultipleForeignKeyConstraintToSQLStatement((AddMultipleForeignKeyConstraint) change);
		} else if (change instanceof AddPrimaryKeyConstraint) {
			return this.transferAddPrimaryKeyConstraintToSQLStatement((AddPrimaryKeyConstraint) change);
		} else if (change instanceof AlterColumnAddConstraint) {
			return this.transferAlterColumnAddConstraintToSQLStatement((AlterColumnAddConstraint) change);
			...
			return this.transferAlterColumnDropConstraintToSQLStatement((AlterColumnDropConstraint) change);
		} else if (change instanceof AlterColumnDropDefault) {
			return this.transferAlterColumnDropDefaultToSQLStatement((AlterColumnDropDefault) change);
		} else if (change instanceof AlterColumnSetDefaultValue) {
			return this.transferAlterColumnSetDefaultValueToSQLStatement((AlterColumnSetDefaultValue) change);
			...
			return this.transferDropComplexUniqueConstraintToSQLStatement((DropComplexUniqueConstraint) change);
		} else if (change instanceof DropForeignKeyConstraint) {
			return this.transferDropForeignKeyConstraintToSQLStatement((DropForeignKeyConstraint) change);
			...
			return this
					.transferDropMultipleForeignKeyConstraintToSQLStatement((DropMultipleForeignKeyConstraint) change);
	...
	protected void prepareSQLStatementForChangeOperation(List<AbstractChangeOperation> changes,
			AbstractChangeOperation change) {
	 ...
	 */
	protected void prepareCreateTableToSQLStatement(List<AbstractChangeOperation> changes, CreateTable change) {
	 ...
	 * @changed OLI 15.12.2015 - Added.
	 */
	protected String transferAddForeignKeyConstraintToSQLStatement(AddForeignKeyConstraint change) {
		...
		if (change.getColumns().length == 1) {
			...
			s = "ALTER TABLE " + this.metaUtil.quote(c.getTable()) + " ADD CONSTRAINT " + this.metaUtil.quote(fkName)
					+ " FOREIGN KEY (" + this.metaUtil.quote(c) + ") REFERENCES " + this.metaUtil.quote(rc.getTable())
	 ...
	 * @changed OLI 15.12.2015 - Added.
	 */
	protected String transferAddMultipleForeignKeyConstraintToSQLStatement(AddMultipleForeignKeyConstraint change) {
		if (!this.foreignKeyConstraints) {
			return null;
		}
		...
		s = "ALTER TABLE " + this.metaUtil.quote(c[0].getTable()) + " ADD CONSTRAINT " + this.metaUtil.quote(fkName)
				+ " FOREIGN KEY (" + this.getQuotedCommaSeparatedColumnNames(c) + ") REFERENCES "
				+ this.metaUtil.quote(rcs[0].getTable()) + " (" + this.getQuotedCommaSeparatedColumnNames(rcs) + ")";
		return s;
	}
	 ...
	 */
	protected String transferAddPrimaryKeyConstraintToSQLStatement(AddPrimaryKeyConstraint change) {
		MetaDataTable t = change.getTable();
		return "ALTER TABLE " + this.metaUtil.quote(t) + " ADD CONSTRAINT "
				...
				+ " PRIMARY KEY ("
				+ this.metaUtil.getCommaSeparated(this.metaUtil.getNames(t.getPrimaryKeyMembers()), true) + ")";
	}
	 ...
	 */
	protected String transferAddComplexUniqueConstraintToSQLStatement(AddComplexUniqueConstraint change) {
		MetaDataUniqueConstraint uc = change.getUniqueConstraint();
		if (uc instanceof MetaDataUniqueWithNullableConstraint) {
			MetaDataUniqueWithNullableConstraint ucn = (MetaDataUniqueWithNullableConstraint) uc;
			return "CREATE UNIQUE INDEX " + this.metaUtil.quote(uc.getName()) + " ON "
					+ this.metaUtil.quote(uc.getTable()) + " ("
					+ this.metaUtil.getCommaSeparated(this.metaUtil.getNames(uc.getColumns()), true) + ") WHERE "
					+ this.metaUtil.quote(ucn.getNullable()) + " IS"
					+ (uc.getName().endsWith("_nullable") ? "" : " NOT") + " NULL";
		}
	 ...
	 * @changed OLI 15.12.2015 - Added.
	 */
	protected String transferAlterColumnAddConstraintToSQLStatement(AlterColumnAddConstraint change) {
		MetaDataColumn c = change.getColumn();
		MetaDataTable t = change.getTable();
		ColumnConstraintType type = change.getConstraintType();
		String s = null;
		if (type == ColumnConstraintType.NOT_NULL) {
			s = "ALTER TABLE " + this.metaUtil.quote(t) + " ALTER COLUMN " + this.metaUtil.quote(c) + " SET NOT NULL";
			...
			s = "ALTER TABLE " + this.metaUtil.quote(t) + " ADD CONSTRAINT "
					...
					+ " UNIQUE ("
					+ this.metaUtil.getCommaSeparated(this.metaUtil.getNames(new MetaDataColumn[] { c }), true) + ")";
		}
		return s;
	}
	...
	protected String transferAlterColumnDataTypeToSQLStatement(AlterColumnDataType change) {
		...
		String s = "ALTER TABLE " + this.metaUtil.quote(t) + " ALTER COLUMN " + this.metaUtil.quote(c)
				+ " SET DATA TYPE " + type;
		return s;
	}
	 ...
	 * @changed OLI 15.12.2015 - Added.
	 */
	protected String transferAlterColumnDropConstraintToSQLStatement(AlterColumnDropConstraint change) {
		MetaDataColumn c = change.getColumn();
		MetaDataTable t = change.getTable();
		if (change.getConstraintType() == ColumnConstraintType.NOT_NULL) {
			return "ALTER TABLE " + this.metaUtil.quote(t) + " ALTER COLUMN " + this.metaUtil.quote(c)
					+ " DROP NOT NULL";
			...
			return "ALTER TABLE " + this.metaUtil.quote(t.getName()) + " DROP CONSTRAINT " + this.metaUtil.quote(
					this.nameGenerator.getUniqueName(t.getName(), this.metaUtil.getNames(new MetaDataColumn[] { c })));
		}
		return "/* WARNING: CONSTRAINT TYPE NOT PROCESSED FOR TABLE: " + t.getName() + ", TYPE="
				+ change.getConstraintType() + "*/";
	}
	 ...
	 * @changed OLI 15.12.2015 - Added.
	 */
	protected String transferAlterColumnSetDefaultValueToSQLStatement(AlterColumnSetDefaultValue change) {
		MetaDataColumn c = change.getColumn();
		String defaultValue = c.getDefaultValue();
		if (defaultValue == null) {
			return "/* WARNING: DEFAULT SET WITH NULL VALUE IS INVALID FOR FIELD: " + c.getFullName() + " */";
		}
		MetaDataTable t = change.getTable();
		return "ALTER TABLE " + this.metaUtil.quote(t) + " ALTER COLUMN " + this.metaUtil.quote(c) + " SET DEFAULT "
				+ this.getDefaultValue(c);
	}
	...
	protected String transferAlterSequenceToSQLStatement(AlterSequence change) {
		...
		return "/** WARNING: VALUE OF SEQUENCE CHANGED (ALTE VALUES NOT POSSIBLE): " + s.getName() + ", StartValue: "
				+ s.getStartValue() + ", IncrementBy: " + s.getIncrementBy() + " */";
	}
	...
	protected String transferCreateIndexToSQLStatement(CreateIndex change) {
		...
		return "CREATE INDEX " + this.metaUtil.quote(i) + " ON " + this.metaUtil.quote(i.getTable()) + " ("
				+ this.metaUtil.getCommaSeparated(i.getColumnNames(), true) + ")";
	}
	...
	protected String transferCreateSequenceToSQLStatement(CreateSequence change) {
		MetaDataSequence s = change.getSequence();
		return "CREATE SEQUENCE " + this.metaUtil.quote(s) + " INCREMENT BY " + s.getIncrementBy() + " START WITH "
				+ s.getStartValue();
	}
	...
	protected String transferCreateTableToSQLStatement(CreateTable change) {
		...
		for (MetaDataColumn c : columns) {
			...
			/*
			 * if (this.notNullConstraints && (c.isNotNull() || c.isPrimaryKey())) { s +=
			 * " NOT NULL"; }
			 */
	 ...
	 * @changed OLI 17.12.2015 - Added.
	 */
	protected String transferDropComplexUniqueConstraintToSQLStatement(DropComplexUniqueConstraint change) {
		MetaDataUniqueConstraint uc = change.getUniqueConstraint();
		if (uc instanceof MetaDataUniqueWithNullableConstraint) {
	 ...
	 * @changed OLI 16.12.2015 - Added.
	 */
	protected String transferDropForeignKeyConstraintToSQLStatement(DropForeignKeyConstraint change) {
		if (!this.foreignKeyConstraints) {
			return null;
		}
		MetaDataColumn c = change.getColumns()[0];
	 ...
	 */
	protected String transferDropMultipleForeignKeyConstraintToSQLStatement(DropMultipleForeignKeyConstraint change) {
		if (!this.foreignKeyConstraints) {
			return null;
		}
	 ...
	 * @changed OLI 21.12.2015 - Added.
	 */
	protected String transferDropPrimaryKeyConstraintToSQLStatement(DropPrimaryKeyConstraint change) {
		MetaDataTable t = change.getTable();
		if (t.getPrimaryKeyMembers().length == 0) {
			log.warn("PRIMARY KEY REQUIRED ALTHOUGH NO PRIMARY KEY IS " + "DEFINED FOR TABLE: " + t.getName());
			return "";
		}
		return "ALTER TABLE " + this.metaUtil.quote(t) + " DROP CONSTRAINT " + this.metaUtil.quote(
				this.nameGenerator.getPrimaryKeyName(t.getName(), this.metaUtil.getNames(t.getPrimaryKeyMembers())));
	}
