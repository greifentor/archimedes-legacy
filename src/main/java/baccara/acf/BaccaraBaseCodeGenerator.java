...
/**
 * A basic class for all Baccara code generators.
 *
 * @author O.Lieshoff
 *
...
abstract public class BaccaraBaseCodeGenerator extends AbstractCodeGenerator {

    /**
     * @changed OLI 29.09.2013 - Added.
     */
    @Override public String createAbsoluteFileName(IndividualPreferences individualPreferences,
            DataModel model, String subProjectName, String className, String packageName,
            boolean specialAPI, TableModel tm) {
        String p = tm.getDataModel().getBasicCodePath() + "/"
                + this.getPackageName(tm).replace(".", "/") + "/" + this.getClassName(tm)
                + "." + this.getFileSuffix();
        if (individualPreferences.getBaseCodePath(model.getName()) != null) {
            p = FileUtil.completePath(individualPreferences.getBaseCodePath(model.getName()))
                    + p;
        }
        return p;
    }
     ...
     */
    @Override public String getEnumClassName(DomainModel dm) {
     ...
     * @changed OLI 06.07.2016 - Added.
     */
    @Override public String getJavaType(ColumnModel column, boolean referencesToKeyClass,
            boolean timestampsWrapped) {
        ...
        return this.typeUtil.getJavaType(column, referencesToKeyClass, timestampsWrapped,
                isEntityGenerator(), this.parameterUtil, this.generatorUtil, this.imports);
    }
     ...
     * @return An array of the tables whose data records are included in a list by the passed
     *         table or an empty array if no list inclusions are detected.
     ...
     */
    @Override public String getOrgApacheCommonsLangBuilderPackageName() {
     ...
     */
    @Override public String getPackageName(TableModel tm) {
        String bpn = tm.getDataModel().getBasePackageName() + ((tm.getCodeFolder() != null)
                && (tm.getCodeFolder().length() > 0) ? "/" + tm.getCodeFolder() : "") + "/"
                + this.getSubPackage();
        return bpn.replace("\\", "/").replace("/", ".");
    }

    /**
     * Returns the plural form of the passed string. If the string contains more than one word,
     * only the last one will be set to its plural form.
     *
     ...
     * @changed OLI 05.04.2016 - Added.
     */
    @Override public String getSrcSubPath() {
        return null;
    }
     ...
     * @changed OLI 05.04.2016 - Added.
     */
    @Override public String getSubProjectFolder(DataModel dm) {
        return null;
    }
     ...
     * @changed OLI 05.04.2016 - Added.
     */
    @Override public String getSubProjectSuffix(DataModel model, TableModel tm) {
        return null;
    }
     ...
     * @changed OLI 05.04.2016 - Added.
     */
    @Override public boolean isEnum(DomainModel domain) {
